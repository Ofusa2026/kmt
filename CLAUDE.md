...
- **書類を組み立てる入口は `buildJobSheetDoc(j)` の1箇所**（分類を見て振り分ける）。
  ...
  - 🎓📋 **高度人材・一般はハローワークの求人票と同じ詰め方**＝見出しごとの枠の中に
    **1行に2〜4項目ずつならべる**（`JS_GRID`＝12等分）。**A4縦1枚におさまる**
    - **項目の幅を決めるのは `_jsUnits(f)` の1箇所だけ**＝ **1/3・2/3・全幅の3つだけ**。
      ⚠️ バラバラの幅にすると**たての線が合わず、そろっていないように見える**（実際に見えた）。
      ラベルの帯も **`JS_LW_FIX` の1つの幅**にそろえてある
    ...
    - 🔠 **文字の設定は `JS_FS_L` / `JS_FS_V` ＋ `JS_LW_W` / `JS_LH` / `JS_PAD` ／
      `JS_VLH` / `JS_VPAD` の一かたまりだけ**（項目名 9.5px・太さ600・行間1.5 ／
      記入欄 9.5px・行間1.35）。
      ⚠️ **項目名を太字＋小さめにすると窮屈に見える**（実際に「余裕がなさそう」と言われた）ので、
      太字をやめて行間と余白を広げてある。**小さくすると1枚に収まるが読みづらい**ので、
      いまは**読みやすさを優先して2枚**にしてある
    - ⚠️ **項目名と記入欄は1行の高さが違う**ので、`_jsCellH()` は**それぞれ別に数えて高いほう**を採る。
      まとめて1つの行高で見積もると紙からはみ出す（実際にはみ出した）
    - 📐 **紙の収まりを直すのは `JOB_SHEET_COL_PX`（920）の1箇所だけ**。
      文字の設定を変えたら、**空欄・10字・25字・40字・60字**を流し込んで
      **どのページも A4（1123px）を超えないこと**を必ず測ってから決める
...
- **確認事項（質問・選択肢・出す条件・必須かどうか）は `REN_CHECK_ITEMS` の1箇所だけ。**
  ...
  - 🎓 **5年満了は `REN_Y5_PLANS` ＋ `_renIs5y(v)` の2箇所だけ**。
    1つめの質問の選択肢に【5年満了】があり、それを選ぶか
    ［☑ この人材は5年満了］（`type:'check'` ＝ `renRcToggle()`）を押すと、
