...
- 期限カード（特例期間中／1・2・3ヶ月以内）、脱退一時金、退職予定日超過、在留情報未入力は
  従来どおり一覧フィルターに追従する
- ⏳ **「特例期間中」の枠が赤く点滅する条件は `_tokureiUrgent(list)` の1箇所だけ**
  ＝ 申請ステータスが `TOKUREI_URGENT_APPLY`（審査中・審査完了）のまま、
  特例期間の残りが `TOKUREI_URGENT_DAYS`(15)日を切っている人材が1名でもいるとき
  （特例期間を過ぎると在留の根拠が無くなるため）
  - 枠に出す文章は **`_tokureiUrgentText()` の1箇所**（「⚠️ 特例期間 あと◯日 の人材が ◯名 います」。
    ◯日は**残りがいちばん短い人**）。点滅は既存の `.due-over`
  - **一覧（`toggleAlertList`）も同じ `_tokureiUrgent()` を見る**ので、件数と中身が必ず一致する。
