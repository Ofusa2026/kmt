...
- **決めるのは `_drWorkerIsNew(appType)` の1箇所だけ**
  ＝ **認定・空欄** なら新規登録／**変更・更新・1-6修正** なら同じ氏名＋企業の行を探して申請情報を更新
- **確認ダイアログの文面は `_drWorkerConfirmText(data)` の1箇所だけ**。
  `saveDecisionAsWorker()` の分岐と**必ず同じ `_drWorkerIsNew()` を見る**
  （聞き方と実際の動きが食い違わないようにするため）
  - ⚠️ **「追加しますか？」とだけ聞かないこと。** 既存人材のときに**新しく増える**と誤解される（実際に言われた）。
    申請種別で見出しを変え（【新規登録】／【既存人材の更新】）、**氏名と企業も出す**
