...
- **`showScreen` では「バナーを描く → 既読にする → バッジを更新」の順を変えないこと**
  （先に既読にすると新着の件数が0になる）
- 🔽 **アラートが多い画面は、新着があっても最初はたたんだ状態で出す**（見出しの「新着 ◯件」だけ。
  押すと開く）。**たたむ画面は `UPDATE_BAR_MIN_SCREENS` の1箇所だけ**（判定は `_updBarStartOpen()`）。
  ⚠️ 最初から開くと下のアラートが押し下げられて集中できない（実際に言われた）。いまは 外国人材（`workers`）だけ
