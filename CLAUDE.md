...
- 自動で入るもの（**きまりは `REG_MEET_ROLES` の下の1かたまり**にまとめてある）
  ...
  - **①監督者の氏名及び役職**（5-6号）… **`_meetSvName()` の1箇所**が、上から順に
    ①`companies.supervisor_name`（＋役職）／②`rep_name`＋（`rep_title` 無ければ `MEET_SV_TITLE_DEF`）／
    ③`contact_name`・`contact_persons[0]`＋（`dept` 無ければ `MEET_SV_TITLE_CONTACT`）で作る。
