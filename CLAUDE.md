...
- **書類を組み立てる入口は `buildJobSheetDoc(j)` の1箇所**（分類を見て振り分ける）。
  ...
  - 🎓📋 **高度人材・一般は特定技能の求人票と同じ「1ページ2列」**（`JS_COL_W` / `JS_COL_GAP` の1箇所）。
    ...
    - 🔠 **文字の設定は `JS_FS_L` / `JS_FS_V` ＋ `JS_LW_W` / `JS_LH` / `JS_PAD` ／
      `JS_VLH` / `JS_VPAD` / `JS_VAL_MIN` の一かたまりだけ**
      （項目名 8.5px・太さ600・行間1.3 ／ 記入欄 9px・行間1.3・いちばん低い高さ10px）。
    ...
    - 📐 **紙の収まりを直すのは `JOB_SHEET_COL_PX`（905＝1列ぶん）の1箇所だけ**。
