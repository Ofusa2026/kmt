...
- **枝番（`folder_no` に `-`）の企業は親にまとめる**＝工場別に請求書を分けない
  ...
  **会社名では判定しない**。親になる行は「枝番でない → 請求法人が入っている → 請求タイミングが入っている」の順で選ぶ
...
| 請求日・売上計上日 | その月の末日 |
| お支払期限 | **翌月末**を基準に、振込＝そのまま／口座振替＝+4日（`MF_PAY`）。`billing_timing` に「翌々◯日払い」と書いてあればそちらが優先（内村組・カワイ＝+10日／澤幡製作所＝+5日） |
...
- 出す前の⚠️は **`_mfWarnings()` の1箇所**（送付方法が空／請求法人が空／請求サイクルが空／
  ...
  **`items` を付けると［▼ 一覧を見る］がその場に開き、行を押すとその人材・企業が開く**
  ...
  - ⚠️ **未確定を数える相手は `feeMgmtWorkers`（CSVに出る人ぜんぶ）。`_fmVisibleWorkers()` を見ない**
    （表を絞り込んでいるときに、件数と一覧の中身が食い違う）
