...
- **書類を組み立てる入口は `buildJobSheetDoc(j)` の1箇所**（分類を見て振り分ける）。
  ...
  - 🎓📋 **高度人材・一般はハローワークの求人票と同じ詰め方**＝見出しごとの枠の中に
    **1行に2〜4項目ずつならべる**（`JS_GRID`＝12等分）。**空欄の状態でA4縦1枚におさまる**
    - 📐 ⚠️ **項目名は記入欄の「上」に置く（たて積み）。`fCell()` の1箇所**。
      横に並べる（項目名｜記入欄）と、項目名が長いぶん**記入欄がつぶれ**、
      項目名も2〜3行に折り返して**紙が2枚に伸びる**（実際に伸びた）。
      上に置くと項目名は欄いっぱいの幅を使えるので**1行におさまり、記入欄も欄いっぱい**になる
    - **項目の幅を決めるのは `_jsUnits(f)` の1箇所だけ**＝ **1/4（3）・1/3（4）・1/2（6）・
      全幅（12）**。答えが短い項目は 1/4 で、**項目名が1行に入らないときだけ広げる**
      （2行になると、その行にならぶ欄すべてが高くなるため）。
      文章を書く項目（`JS_LONG_RE` / textarea）だけ 1/3 にする。
      項目ごとに決めたいときは定義に `w:` を書けばそれが優先される
      紙の収まりは **`JOB_SHEET_COL_PX`（1ページに入る中身の高さ px）**
    ...
    - 🔠 **文字の設定は `JS_FS_L` / `JS_FS_V` ＋ `JS_LW_W` / `JS_LH` / `JS_PAD` ／
      ...
      太字にはしない
    - ⚠️ **たて積みなので、欄の高さ＝項目名のぶん＋記入欄のぶん**（`_jsCellH()`）。
      1行の高さは**その行でいちばん高い欄**で決まるので、
      **1つでも項目名が2行になるとその行ぜんぶが高くなる**（だから幅を広げて1行にする）
    - 📐 **紙の収まりを直すのは `JOB_SHEET_COL_PX`（1000）の1箇所だけ**。
