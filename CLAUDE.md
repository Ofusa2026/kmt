...
- 📌 **企業を開いたときの上の2つ（← 企業名 ／ 企業全体のチャット）は一番上に貼り付ける**
  （`renderGroupRooms` の中の `_chatStickTop` の1箇所）。人材が多い企業だと下までスクロールしたとき、
  どの企業を見ているのか分からなくなり、企業全体のチャットにも戻れなかったため
  - ⚠️ **2つまとめて1つの箱に入れて `position:sticky` にする**（別々に貼ると、
    上の高さを測って2つめの `top` を出すことになる＝計算を2つ持つ）
  - ⚠️ 箱には**必ず背景色**を付ける（透けると下の行が重なって見える）
  - 企業チャットがまだ無いときの［💬 企業チャットを開始］も同じ箱に入れる
...
- **添付**: `chat_messages.attachments`（`[{url,name,mimeType,size}]`）。実体は GAS 経由で Google Drive にアップロード。
  ...
  - 🩺 **「返事が返ってこなかった」を「入れられなかった」と決めつけないこと。
    判定は `_chatDriveNoReply(e)` の1箇所だけ**（`Failed to fetch` / `NetworkError` /
    タイムアウト等）。**ドライブへの書き込みは終わっているのに、返事だけ受け取れない**ことがある
    - > ⚠️ **過去の事故**: 2026/09/15 に「ドライブに1件も入れられませんでした／
      > KMT Drive：Failed to fetch」と出たのに、**ファイルは両方ドライブに入っていた**。
      > そのまま入れ直すと**同じファイルが二重に入る**
    - このときは見出しも本文も**「入ったかどうか分かりませんでした」**にして、
      フォルダを開いて確かめてもらう。**入れ直させない**
    - 🔑 **いちばん多い原因はフォルダの共有設定**（`RECEIPT_SHARE_HINT` を一緒に出す）。
      ドライブ側は「ファイルを作る」→「リンクで見られるようにする」の順に動くので、
      **後半で止まるとファイルは入ったまま返事だけ返らない**＝領収書と同じ原因・同じ直し方
      - ⚠️ **大きさは関係ない**（1.05MB のPDFで実際に起きた）。
        `CHAT_FILE_WARN_MB` の警告は別の保険で、これが原因ではない
    - 🔗 逃げ道は **`_chatAttachByPastedLinks(fails)` の1箇所だけ**＝
      ドライブに入っていたら**そのファイルのリンクを貼れば添付できる**
      （アップロードし直さない）。**ファイル名ごとに1回だけたずねる**
      （同じファイルが「KMT側」「大房側」と複数行で並ぶため）
...
- 📎 **添付の入れ先を決めるのは `_tcRoomDriveUrl(r)` の1箇所だけ**＝
  そのグループに**専用フォルダー**（`team_chat_rooms.drive_url`）があればそこ、無ければ共通のチャット用フォルダー。
  人材・企業のドライブは使わない（社内の話なので置き場を分ける）
  - 📁 **グループごとの専用フォルダー**は ⚙️ 設定の欄（`tcf_drive`）にフォルダーのURLを貼るだけ。
    **欄の下の1行（`renderTcDriveHint()`）も送信と同じ `_driveFolderUrl()` で見る**ので、
    「入ります」と出たものは必ずそこに入る（⚠️ 欄に `なし` のような文字が入っても落ちない＝
    フォルダIDが取れなければ共通のフォルダーに入る）
  - 設定してあるグループは**チャットの上に［📁 フォルダー］**が出て、
    ファイルを選んだときも添付の欄に「→ ◯◯ 専用フォルダー に入ります」と出る
  - ⚠️ **GAS の接続先は、ほかの添付と同じく `localStorage.kmt_gas_url` → 無ければ `KMT_DRIVE_GAS_URL`**
    （`_tcUploadFiles` の1箇所）。以前は既定値を持っておらず、設定を開いたことのない人は
    **「GAS URL または保存先が未設定です」で必ず送れなかった**（実際に送れなかった）
  - ⚠️ **専用フォルダーは `RECEIPT_GAS_ACCOUNT`（kmt@k-m-t.jp）に編集者で共有されていないと入らない**
    （ドライブが「指定した ID のアイテムは見つかりませんでした」で落とす）。
    **「リンクを知っている全員（編集者）」の共有にはしない**（領収書と同じ理由）
  - ⚠️ **アイコン画像は専用フォルダーに入れない**（`_tcUploadFiles(_tcRoomDriveUrl(null))` を渡す）＝
...
- 👀 **既読は `team_chat_members.last_read_at` の1箇所**（書くのは `_tcMarkRead()` だけ）
  - ✓✓ **発言ごとの既読（人数とだれか）は `_tcMsgReaders(m)` の1箇所だけ**＝ そのルームのメンバーのうち、
    ...
    新しいテーブルは作らない
    - 発言の下に「✓✓ 既読 2 / 5」（全員なら「全員既読」）＝ `_tcReadReceiptHtml(m)`。
      押すと `showTcReaders()` が **読んだ人（時刻）と、まだ読んでいない人**を出す
      （メンバーが決まっているので未読の人も出せる＝💬 チャットとちがうところ）
    - 12秒ごとの読み直し（`loadTcMessages`）で**開いているルームのメンバーも読み直す**。
      ほかの人が読んだだけのときは**スクロールを動かさず描き直すだけ**（既読にし直さない）
    - ⚠️ **変わったかの印（`_tcReadSig`）に自分の行を入れない**＝ 開いているあいだ自分の既読は毎回進むので、
      入れると12秒ごとに描き直し・既読の書き込みが続く
...
- 💬 **発言の削除は論理削除**（`deleted_at`）＝ 一覧には「🗑️ このメッセージは削除されました」と残す。
  消せるのは**自分の発言だけ**
- ✏️ **発言の編集**（［✏️ 編集］→ その場の入力欄 →［💾 保存］／Ctrl+Enter）。
  **できるかどうかは `_tcCanEdit(m)` の1箇所だけ**＝
  **自分の発言はだれでも** ／ **自動メッセージ（`team_chat_messages.is_auto`＝グループの説明など）は管理者**
  （だれの名前で入っていても直せる。いまは総務の2グループの説明に `is_auto` を付けてある）。
  ⚠️ **自動メッセージを新しく入れるときは `is_auto=true` を付けること**（付けないと入れた本人しか直せない）
  **同じ行の `content` ＋ `edited_at` を書き替えるだけ**（行を増やさない＝既読・未読の数は変わらない）。
  @メンションは送信と同じ作り方（`extractMentions` → `expandMentions` → `_withReplyMention`）で付け直す。
  時刻の横に「（編集済み）」を出す
  - 📜 **編集履歴は `team_chat_message_edits`（1回の編集＝1行：変更前・変更後・だれが・いつ）**。
    **見られるのは管理者だけ**＝「（編集済み・履歴）」を押すと `openTcEditHistory()` が開く（`requireAdmin()`）。
    管理者以外には「（編集済み）」の印だけを出す
  - ⚠️ **保存は「履歴を書く → 本文を直す」の順。履歴が書けなかったら本文も直さない**（`tcSaveEdit`）＝
    履歴の残らない編集を作らない。直した内容は入力欄に残る
  - 履歴の無い古い編集（ver.20260924.1 のあいだ）は、履歴の画面に「中身が記録されていません」と出す
  - **いま編集している発言は `_tcEditingId` の1箇所だけ**（入力中の文字は `_tcEditDraft`）
  - ⚠️ **編集中は12秒ごとの読み直しで描き直さない**（`loadTcMessages` の中の1箇所）＝
    描き直すと入力欄が作り直されて、打っている途中の文字が消える
  - 直したまま別のルーム・画面へ移るときは1回たずねる（`_tcEditLeaveOk()` ／ `showScreen` の中）
- ☑ **メールのように選んで既読・未読にする**（一覧の上の［☑ 選んで既読・未読にする］→ チェック →［✓ 既読にする］／［● 未読にする］）。
  ［すべて選択］は**いま一覧に出ている・参加しているルームだけ**（絞り込み・検索にそのまま従う）。
  開いているチャットはヘッダーの［● 未読にする］でも未読に戻せる
  - **書き替えるのは `tcMarkRooms(ids, unread)` の1箇所だけ**で、さわるのは**自分の `team_chat_members.last_read_at` だけ**
    （発言そのもの・ほかの人の既読・💬 チャット／💬 求人チャットの `chat_read_at` には一切ふれない）
    - 既読 … いまの時刻／未読 … **ほかの人のいちばん新しい発言の 1ms 前**（＝その1件が未読＝件数 1）。
      ほかの人の発言が1件も無いルームは未読にできない（トーストで件数を知らせる）
  - 選んでいるのは **`_tcSel` の1箇所**（選択モードは `_tcSelMode`）。**参加していないルームは既読の記録が無いので選べない**
  - ⚠️ **未読に戻した発言は `_tcNotifSeen` に入れる**（入れないと、未読に戻しただけでブラウザ通知が鳴る）
  - ⚠️ **開いているチャットを未読にしたら閉じる**（開いたままだと読み直しですぐ既読に戻るため）
  - 数え直しは `loadTeamChatUnread()` にまかせる＝ 一覧・サイドバーのバッジ・ショートカットが必ず同じ数になる
- ⚠️ **`showGenModal` は本文しか受け取らない**ので、モーダルのフッターは
  **`_tcFooterHtml()` の1箇所**で本文の最後に足す
- 🔴🔵 **自分あて／全体あての分け方は `_tcMentionKind(m)` の1箇所だけ**
  （`''`／`'direct'`🔴／`'all'`🔵）。合計は `_tcMentionDirectTotal` / `_tcMentionAllTotal`
  - ⚠️ **見る順番がそのまま優先順位**（💬 チャットの未読集計とそろえてある）＝
    ① 本文に @自分の名前（`_mentionNamedInText()`）→ 🔴 ② @全員（`isMentionToAll()`）→ 🔵
    ③ それ以外で `mentions` に自分がいる → 🔴
  - ⚠️ **③を①より先に見てはいけない**＝ 本文が「@全員」だけの投稿でも `mentions` には
    全員の名前が入っているので、**全部が🔴自分あてになってしまう**
  - そのため未読集計の GET には **`content` も入れてある**（`mentions` だけでは分けられない）
- ↗ **左メニューの「💬 求人チャット」のすぐ下に、社内チャットのショートカットを1行出す**
  （`#teamChatShortcut`。押すと `showScreen('team_chat')`）
  - **出すかどうかを決めるのは `updateTeamChatShortcut()` の1箇所だけ**＝
    🔴自分あて ＋ 🔵全体あて が1件でもあるとき。呼ぶのは `updateTeamChatBadge()` の最後
  - ⚠️ **💼 求人チャットと違い、🔵全体あて（@全員）だけのときも出す**
    （社内の連絡はそれ自体が用件で、見落とすと困るため。実際にそう頼まれた）
  - ⚠️ **件数は数え直さない**＝ 上の2つの合計をそのまま見るので、
    一覧の @バッジ・ナビのバッジと**必ず同じ数**になる
...
#### 📋 紹介案件の決定報告も **ボタン1つで全部やる**（`sendIntroDecisionAll(kind)`）

- **入口は `sendIntroDecisionAll(kind)` の1箇所だけ**（他社支援・企業単独とも
  ［📋 決定報告を作成（全部まとめて）］の1つ）。
  **押すと先に「何がどこに反映されるか」が出て、［✅ OK］のときだけ進む**
  - **確認の文面を作るのは `_introDecisionPlanHtml(kind, rows)` の1箇所だけ**（`askChoice()` で出す）。
    ⚠️ **実際に走る処理と同じ値・同じ条件から作ること**＝ `_drIntroRowsOf(kind)` の結果と
    `DR_INTRO_SALE` をそのまま見る。企業が所属機関に無い／登録支援機関が提携先に無い／
    金額が空のときは赤で知らせる
  - やることは4つ ＝ ①文面をコピー ②控えを `decision_report_log` に保存
    ③🤝（紹）決定者リスト ④💰 売上（③④は `linkIntroDecisionFromReport()` の1箇所）
- > ⚠️ **過去の事故**: 以前は［📋 テキスト生成・コピー］と［🤝 決定者リスト＋売上に登録］の
  > **2つのボタンに分かれていて、コピーだけ押して終わっていた**（KMT案件とまったく同じ事故）。
  > **押し分けを無くすためにボタンを1つにした。増やし直さないこと**
- 📥 **「大房側へ依頼」の押し忘れは `_drOfusaRemind(kind)` の1箇所**がたずねる
  （［📥 大房側へ依頼する］／［閉じる］）。押した日時は **`_drOfusaSent` の1箇所**に覚える
  （`sendDecisionFormToOfusa` が成功したときに入れる。画面を開き直すと空に戻る）
- ⚠️ **`_saveDecisionReportLog()` は同じ控えを2件作らない**＝ 1つの決定報告で
  ［📋 決定報告を作成］と［📥 大房側へ依頼］の**両方**を押すので、
  **人材・受付日・kind が同じ控えがあれば作らない**（見るのはこの1箇所だけ）

#### 🏢 決定報告（他社支援）の【登録支援機関名】＝ 提携先から選ぶ

- **入れ物は今までどおり `ot_support_org` の1つだけ**（手で打つこともできる）。
  ピッカーはそこに名前を入れるだけなので、文面・大房送信・決定者リストは何も変えていない
- **選んだ提携先の番号は `_drOrgNo` の1箇所**に覚え、控え
  （`decision_report_log.org_no` / `org_name`）に残す＝ 提携先の【📋 決定報告の履歴】はこれを見る
- **［＋ 新規提携先］は求人登録と同じ `createIntroRegOrg()`**（登録のしかたを2つ持たない）
- 依頼元（`of_ot_agency`）には選んだ名前をそのまま写す。
  ⚠️ **手で直されたぶんは上書きしない**（写した文字を `_drOrgMirror` に覚えておく）
- ⚠️ `pickDrCandidate()` は `change` しか出さないので、**流し込んだあと `onDrOrgInput()` を1回通す**
  （`oninput` で見ているため）

#### 📋 提携先登録支援機関の【決定報告の履歴】タブ（3つめ）

- **読むだけの画面。元データは `decision_report_log` の1箇所だけ**
- **その提携先のぶんかは `org_no` で見る**（会社名の文字列一致では判定しない）。
  古い控えには `org_no` が入っていないので、**機関名（`org_name`）でも拾う**
- 読むのは `loadIroDecisionLog()`（タブを開いたときだけ）／描くのは `renderIroDecisionLog()`／
  1件は `openIroDecisionDetail(i)`（そのときの文面をそのまま出す・コピーできる）
...
- `ph` は記入例（欄の中に薄く「例）…」と出す）、`note` は法令根拠・留意点（欄の下）、
  ...
  - ✏️ **使っている項目にはすべて `ph`（記入例）を付ける**＝ 外部の記入フォーム（`_jobFormExtFieldHtml`）にも同じ記入例が出る。
    項目を足すときも `ph` を忘れずに書く
  - ⚠️ **記入例には必ず「例）」を付ける**（`_jsExtFieldHtml` の1箇所）。付けないと値が入っているように見える。
    色は `.jsx-f::placeholder` の1箇所で薄くしてある
- 🔘 **「あり／なし」で答えられる項目・雇用形態などは選択式**（項目に `opts:JS_OPT.<名前>` を書く）。
  **選択肢は `JS_OPT` の1箇所だけ**（雇用形態＝正社員／契約社員／パート・アルバイト／派遣社員／嘱託 など）
  - **どの項目にも「その他（記入）」が付く**（`JS_OPT_OTHER`）。選ぶと記入欄（`jsxd_<k>`）が出て、書いた文字をそのまま入れる
  - `{ v:'あり', d:'3か月' }` と書くと、選んだときだけ中身の記入欄も出て **「あり（3か月）」の形**で入る
  - **保存先は今までどおり `sheet_ext[kind][k]` の文字1つ**（列もキーも変えない）。
    読み替えは `_jsOptParse()` の1箇所＝ 選択肢に無い書き方（これまでの自由記入・自動反映の「有期　1年」など）は
    **「その他」の欄にそのまま出して消さない**
  - 欄を作るのは **`_jsOptInputHtml()` の1箇所**（社内の `_jsExtFieldHtml` と外部の `_jobFormExtFieldHtml` が共用）
  - ⚠️ **値を読むのは `_jsxValue(k)` の1箇所だけ**（`_jsExtFormValues` / `_jobFormValues` がこれを通す）。
    `jsx_<k>` は選択欄なので、`.value` をそのまま読むと「__other」が入る
...
- 一覧では「**更新日付 ／ 国籍 ／ 状況**」を**縦に**並べる（例: `9/1　イ　募集中`）
  - > ⚠️ **以前は `job_progress.candidate_status` の1つの文字列に
    > 「7/29 イ：募集中 ／ 6/17ミャ：募集中」と全部入れていた**ので、
    > 一覧が横に長くなって**すぐ確認できなかった**（実際に言われた）
- **国籍の一覧・頭文字・色は `CAND_STATUS_NATS` の1箇所だけ**
  （インドネシア（イ）／ベトナム（ベ）／タイ（タ）／カンボジア（カ）／ミャンマー（ミ）／
  その他（他）＋記入欄）。入力は選択式、**一覧に出すのは頭文字だけ**（国名は title に出す）
- 実体は **`job_progress.candidate_status_rows`**（jsonb の `[{date, nat, nat_other, status}]`）。
  別テーブルを作らない
- ⚠️ **保存するときは 1本の文字列（`candidate_status`）も必ず一緒に書く**＝
  これまでの検索・並べ替え・書き出しがそのまま動くようにするため。
  組み立てるのは **`_csText()` の1箇所だけ**。書くのは2か所
  （`saveJobProgress` / `saveJobProgressNew`）で、どちらも `_csFormRows()` を通す
- **読むのは `_csRows(r)` の1箇所だけ**。`candidate_status_rows` が**配列なら空でもそのまま返し**、
  無いときだけ 1本の文字列を `_csParseText()` で行に分ける
  （⚠️ 空を返さないと、**全部消したのに古い文字列がまた出てくる**）
- **移行はしない**。これまでの文章はそのまま行に分けて表示され、
  開いて保存したときだけその案件が新しい形にそろう
- 📛 **書き方の読み替えは `_csNatOf(token)` の1箇所だけ**
  - ひらがなはカタカナに直す（「べ」＝ベトナム）
  - ⚠️ **見るのは先頭の1文字だけ**。中に入っている文字まで拾うと
    「**タイ**」がインドネシア（イ）に、「スリラン**カ**」がカンボジアになる（実際になった）
  - 先頭に残った日付・記号は落としてから見る（「19 ミャ」→ ミャンマー）
- 🗓 年が書かれていない日付（「9/2」）は **`_csGuessDate()` の1箇所**が
  「今日を過ぎない一番近い年」とみなす
- 一覧のセルを作るのは **`_csCellHtml(r)` の1箇所だけ**（求人管理リストと売上見込みで共通）
- 入力は求人案件の【📋 求人進捗 情報】タブ。編集中の行は **`_jpCsRows` の1箇所だけ**に持つ
  - ⚠️ **1文字打つたびに描き直さない**（フォーカスが飛んで打てなくなる）＝
    `updCandStatusRow()` は配列を書き替えるだけ。描き直すのは
    **国籍を選び直したとき**（記入欄の出し入れがある）と**追加・削除**のときだけ
  - ⚠️ 欄の id が `jpf_` 始まりでないので、**未保存の印は `_csTouch()` の1箇所**で自分で立てる
  - 外側の入れ物の id は **`jpf_candidate_status` のまま**（売上見込みのセルを押したときの
    `openJobProgressDetail(src_row,'jpf_candidate_status')` がそのまま効く）
...
- 🔍 **【👥 候補者リスト】の「候補者を名前で検索」（`#jpCandSearchBox`）は
  `setJpCandSearch(v)` の1箇所**が受ける。**入れ替えるのは `<select>`（`#jpCandPick`）の中身だけ**で、
  中身を作るのは **`_jpCandOptionsHtml()` の1箇所**（全部描き直すときも同じものを使う）
  - ⚠️ **打つたびに一覧ごと（`box.innerHTML`）描き直さない。** 検索欄そのものが作り直されるので、
    **1文字入れただけでフォーカスが外れて続けて打てない**（実際に打てなかった）。
    日本語の変換も途中で切れる
  - ⚠️ 「描き直してからフォーカスを戻す」やり方（`setRenCycleQ` など）より**こちらを優先する**＝
    検索欄に触らないので**カーソルの位置も変換中の文字もそのまま**残る
    （フォーカスを戻すやり方はカーソルが必ず末尾に飛ぶので、途中に文字を足せない）
...
### 📣 求人登録リンク（1本を何社にでも送る求人の登録フォーム）

- 候補者の📣 募集リンクと同じ考え方＝ **案件を先に作らず、企業が送信した瞬間に求人案件（`job_progress`）が1件でき、
  その会社専用の続きリンクも自動で出る**。入口は求人管理の右上［📣 求人登録リンク］（`openJobApplyLinks()`）
- 実体は今までと同じ **`job_sheet_links`**。**見分けるのは `kind` の1つだけ**
  （`'job'`＝案件ごとの記入リンク（今までどおり）／`JOB_APPLY_KIND`＝`'apply'`）。
  求人登録リンクは **`job_src_row` が null**（そのために NOT NULL を外した）。`label`＝社内の名前／`sheet_kind`＝出す項目
  - ⚠️ 案件ごとのリンクを読むところ（`loadJobSheetLinks` / 求人アラート）は `job_src_row` で引くので、求人登録リンクは混ざらない
- **画面は外部記入フォームの `_jobFormHtml()` をそのまま使う**（項目を二重に持たない）。
  求人登録リンクのときだけ変わるところは **`_jobApply`（`ap`）の1つ**で見る
  - **必須は事業所名（法人名）だけ**（`jfaCompany`）。ほかは任意。
    一般・高度人材の様式の「事業所名（法人名）」（**`JOB_APPLY_NAME_KEY`**＝`g8` / `h7`）は様式の中では出さず、送信時にこの欄の値を入れる
  - ⚠️ **1本を何社も使うので、一時保存（draft）は読まない・書かない**（前の会社の内容が出てしまう）。
    冒頭の案内（`_jobFormIntroHtml()`）も「一時保存」の話があるので出さず、`_jobApplyLeadHtml()` が代わりに出る
- 送信は **`submitJobApply()` の1箇所**＝ ① 進捗「詳細確認中」・source **`JOB_APPLY_SOURCE`（`jobapply`）** で案件を作る
  （⚠️ `src_row` は送らない）② その会社専用の続きリンク（`kind='job'`・回答ずみ）を作る
  （`created_by`＝`JOB_APPLY_AUTO_BY`、`label`＝求人登録リンクの `token`）③ 完了画面に続きリンクを出す（`_jobApplyDone()`）
  - 区分は仮に入れる＝ 記入者が【登録支援機関】なら紹介案件、それ以外はKMT案件（精査のときに直す）
  - 🌏 特定技能の数字・日付の列（**`JOB_APPLY_TYPED_COLS`**）は `_jobApplyCoerce()` を通す。
    **読めない値は列に入れず `hearing_note` に残す**（1つでも入らない値があると登録がまるごと落ちるため）
  - 続きリンクは **`seen_at` を入れて作る**＝ 最初の登録は「📣 求人登録リンクから届いた未精査の案件」（`applyRaw`）で知らせ、
    「📬 回答が届いています」には重ねない（あとで追記されたらそちらに出る）
- 登録が何件来たかは**数え直して出す**（続きリンクの `label` を数える。候補者の募集リンクと同じ）
- 🏢 **外部の記入フォーム（求人登録リンク・案件ごとの記入リンクの両方）に出さない項目は `_jobFormHideField(f)` の1箇所だけ**＝
  一般・高度人材は `agent:true`（Ⅰ．受付情報＝紹介事業者が記入）、特定技能は `JOB_FORM_INTERNAL_COLS`（受付年月日・有効期間）。
  番号は**フォームに出している項目だけで1から振り直す**（社内の画面・書類の番号とは別）
- 📅 **求人受付日＝企業から回答が届いた日**。`job_progress.reception_date` に入れる
  （求人登録リンクは必ず／案件ごとのリンクは空のときだけ）。一般・高度人材の「求人受付日」（`g1` / `h1`）は
  `JOB_SHEET_EXT_FILL` が `_jsRecvText(reception_date)` から作る（「2026年9月4日」の形）
- 👁 **閲覧用（下見）は `_jobApplyPreview` の1箇所**（`openJobApplyPreview(kind)` / `closeJobApplyPreview()`）＝
  リンクを発行しなくても同じフォームを確かめられる。**DBには何も書かない**
  （送信・写真の追加・登録支援機関の新規登録を止める）
- 発行・無効にはだれでも、削除は管理者だけ（無効にしたものだけ）
...
#### 📝 履歴書の「選べる項目」（RESUME_OPTS）

- ご本人（特にミャンマーの方）に記入をお願いすると**自由記入では何を書けばよいか分からない**ので、
  よく使う項目を選択式にしてある
- **選択肢は `RESUME_OPTS` の1箇所だけ**（履歴書作成の画面・候補者モーダルの【📝 履歴書作成】タブ・
  候補者情報タブ・本人が書く外部リンクが同時に追従する）

  | キー | 選択肢 |
  |---|---|
  | `jp_language_level` 日本語レベル | N5／N4／N3 |
  | `jp_exam_1` 日本語試験 | なし／JFT A2／JLPT N1〜N4 |
...
- 🎓 **【修了予定日】は在留資格が技能実習２号・３号のときだけ出す。判定は `_jissyuVisa()` の1箇所**
  ...
  - ⚠️ **日付の列を空にするときは必ず `null`。`''` を送ってはいけない**＝
    「invalid input syntax for type date: ""」で**保存がまるごと落ちる**
    （技能実習以外の候補者で履歴書がまったく保存できなくなった。実際に起きた）。
    `getResumeFormData()` の `g()` は空の欄を `null` で返すので、
    **`? … : ''` のように `g()` を通さず空文字を書いたところだけが危ない**
    （`education_note` / `health_note` / `lifestyle` / `night_shift_count` は text なので `''` でよい）
  - 📋 **置き場所は入力欄も書類も「在留資格のすぐ下」**（【📋 在留・連絡情報】の中）。
    在留のことなので【📚 日本語・技能試験】には置かない
- 🈚 **【日本語試験】（`jp_exam_1`）は履歴書から外した**＝【日本語資格】（`jp_language_level`）と
  同じ内容だったため（実際に言われた）。**列も値も消していない**ので、これまで入れた値はDBに残る
  - ⚠️ **`getResumeFormData()` から `jp_exam_1` を送らないこと**（送ると空で上書きする）
  - `RESUME_OPTS.jp_exam_1` は**昔の値を読むためだけに残してある**
- ⚠️ **一覧に無い値（これまでの自由記入）は消さない**。選択欄を作るのは
  **`resumeOptsHtml(key, cur)` の1箇所**で、いまの値が一覧に無ければ先頭に残す
  - ⚠️ **`fillResumeForm` の `g()` も同じことをする。**
    `<select>` に一覧に無い値をそのまま `el.value =` で入れると**空になって値が消える**
    （「N4相当」のような昔の書き方が消えてしまう）。**足すのは `g()` の1箇所だけ**
- 🗣️ **聞き取り・会話は JLPT の N1〜N5 を別々に選ぶ**（`JP_SKILL_LEVELS` / `JP_SKILL_PARTS` の1箇所）。
  **保存先は今までどおり `jp_language_detail` の1列**で「聞き取り:N4 会話:N5」の形。
  読み書きは **`_jpDetailParse()` / `_jpDetailValue()` の2つだけ**を通す
  - ⚠️ 以前は ◎○△✕ にしていたが、**応募フォームでご本人が答えるには分かりにくい**ので
    JLPT のレベルに変えた（実際に言われた）。**移行はしない**＝
...
- 履歴書では**【一時帰国希望】のセルの中に小さく**出す（「タイミング：◯◯ ／ 期間：◯◯」）＝
  行を増やさない（A4縦1枚に収めるため）
- 📋 **入力欄の並びは 一時帰国希望 → タイミング → 期間 → 就労についての家族の同意 の順**＝
  ⚠️ **追加の欄は「その項目のすぐ下」に出す**（以前は2列の枠のうしろにまとめて出していたので、
  家族の同意をはさんだ下に出ていて、どの項目の続きなのか分からなかった。実際に言われた）
- ⚠️ **項目名・選択肢・記入例は `EXT_FORM_I18N` にも足すこと**（本人が読む外部フォームに出る）

#### 👪 就労についての家族の同意（相談中のときだけ聞く1つ）

- **項目名は `FAMILY_CONSENT_LABEL` の1箇所だけ**（`就労についての家族の同意`。入力欄・履歴書が追従する）。
  選択肢は `RESUME_OPTS.family_consent` ＝ 同意している／**相談中**／同意していない
- **【相談中】のときだけ**下に【いつまでに結論が出ますか】（`family_consent_due`）を出す。
  **聞くかどうかの判定は `_familyConsentAsk(v)` の1箇所だけ**で、
  欄の出し入れ（`onFamilyConsentChange()`）・履歴書に出すか（`_familyConsentNote()`）・
  記入必須のチェック（`_familyConsentOk()`）・保存で空にするか が同じものを見る
- 欄を作るのは **`familyConsentFieldHtml(prefix, due)` の1箇所だけ**
  （履歴書作成の画面・候補者モーダルの【📝 履歴書作成】タブ・本人が書く外部リンク・
  📣 募集フォームが同時に追従する）
- ⚠️ **【いつまでに結論が出ますか】は記入必須。判定は `_familyConsentOk(prefix)` の1箇所だけ**で、
  ...
  **病名（`_healthNoteOk()`）とまったく同じ作り**
- ⚠️ **「相談中」以外に切り替えたら保存で空にする**（病名・中退メモと同じ考え方）。
  text の列なので `''` でよい
...
- **選択肢は `LIFESTYLE_OPTS` の1箇所だけ**（タバコを吸う／吸わない・アルコールを飲む／飲まない・
  運動を定期的にする／あまりしていない）。**あてはまるものをいくつでも選べる**
- **保存先は `candidates.lifestyle` の1列**で、選んだものを「、」（`LIFESTYLE_SEP`）でつないで入れる
  （提携先の区分・主な職種と同じ持ち方。列を増やさない）
- **読み書きは `_lifestyleValue(prefix)` / `_lifestyleSet(prefix, v)` の2つだけ**を通す。
  欄を作るのは `lifestyleFieldHtml(prefix, cur)` の1箇所（履歴書作成の画面・候補者モーダルの
  【📝 履歴書作成】タブ・本人が書く外部リンク・📣 募集フォームが同時に追従する）
- ⚠️ **一覧に無い値はチェック済みのまま末尾に残す**（`_lifestyleListHtml()`）＝選び直さなくても消えない
- ⚠️ **候補者情報タブには出していない**（夜勤と同じで履歴書だけの項目）。
  `CAND_SYNC_FIELDS` にも入れていない

#### 🖨 履歴書の【🏥 健康・その他】の出しかた

- 表は **健康状態（＋病名）／ライフスタイル の1行だけ**
- ❓ **本人からの質問は別の枠**（`resume-memo-box`。志望動機と同じ作り）。
  ⚠️ **中身が空のときは枠ごと出さない**（履歴書が1枚に収まらなくなるため）

#### 🎓 学歴（最終学歴・卒業状況）

- **選択肢は `EDU_LEVELS`（中学校〜大学院）／`EDU_STATUSES`（卒業・中退・在学中）の1箇所だけ**。
  保存先は `candidates.education_level` / `education_status`
- 卒業年月しか無いと「2年生で中退」「まだ在学中」が書けないので、
  **どこまで行ったか（最終学歴）と、卒業したかどうか（卒業状況）を分けて持つ**
- ⚠️ **最終学歴に「卒業」を入れない**（`大学` であって `大学卒業` ではない）。
  卒業したかどうかは隣の卒業状況で選ぶので、入れると「**大学卒業 / 中退**」と食い違って見える（実際に言われた）
  - **末尾の「卒業」を落とすのは `_eduLevelText()` の1箇所だけ**＝ 旧データ（`大学卒業` など）が
    入っていても画面・履歴書では「大学」と出る。**移行はしない**（列の値はそのまま）
  - 選択欄を作るのは **`eduLevelOptionsHtml(cur)` の1箇所**で、
    **一覧に無い値（旧データ）はそのまま先頭に残す**（選び直さなくても消えない）
- 卒業状況で**日付の見出しが変わる**（`onEduStatusChange()` の1箇所。中退＝中退年月／在学中＝入学年月）。
  履歴書の学歴の表も同じ見出しで出す。学歴の1行を作るのは `_eduLine()` の1箇所
- 📝 **中退のときだけ自由記入欄を出す**（`rf_education_note_box`。保存先は `candidates.education_note`）。
  **出す卒業状況は `EDU_NOTE_STATUSES` ＋ `_eduNeedsNote()` の1箇所だけ**で、
  欄の出し入れ（`onEduStatusChange`）・履歴書に出すか（`_eduNoteText()`）・保存が同じものを見る
  - 履歴書では **【最終学歴】のセルの中、`_eduLine()` のすぐ下**に小さい字で出る（未記入なら何も出ない）
  - ⚠️ **中退以外に切り替えたら保存で空にする**（`_eduNeedsNote()` を通す）＝
    卒業に直したのに中退の説明が履歴書に残らないようにするため
  - 本人が書く履歴書作成リンクにも出る（項目定義を共用しているため）。
    **`EXT_FORM_I18N` に項目名・記入例・注記も入れてある**
...
### 🔠 提携先の求人コード（ELN27 → 次の候補 ELN28）

- 紹介案件で提携先を選ぶと「最後に使ったコード」と「次に使えるコードの候補」が出て、
  **求人コードが空のときだけ**候補を入れる（`pickJpOrg()`）
- **コードを読むのは 3つの1箇所だけ**
  | 関数 | 中身 |
  |---|---|
  | `_jobCodePrefix(code)` | 接頭辞（`FC01` → `FC`） |
  | `_jobCodeDigits(code)` | 番号の部分を**文字のまま**（`FC07` → `'07'`）＝桁数を数えるのに使う |
  | `_jobCodeNum(code)` | 番号（`FC07` → `7`） |
  - ⚠️ 接頭辞と番号のあいだの**空白・ハイフンは無視する**（`_JOB_CODE_RE` の1箇所）＝
    「ELN 20」「BW 01」のように空けて入っている案件が実際にあり、番号として拾えていなかった
- ⚠️ **番号を数えるのは「その提携先のコードで始まる求人コード」だけ**（`iroLastCode()` の1箇所）。
  `iroCases()` は **登録支援機関コード（`org_code`）が一致するだけ**の案件も集めるので、
  そこに別の接頭辞のコードが混ざっていることがある
  - > ⚠️ **過去の事故**: 株式会社エラン（`ELN`）の案件に **`FC86`**（＝ファーストクラスのコード）が
    > 付いた行が1件あり、そこから番号 86 を拾っていた。さらに桁数を
    > `last.replace(key,'')` で数えていたので `'FC86'.length` ＝ 4桁になり、
    > 次の候補が **`ELN0087`** と出た（正しくは `ELN28`）。
    > **桁数は `_jobCodeDigits()`＝番号の部分だけ**から数える
- ⚠️ **候補はあくまで候補**。手で直せるので、出た番号をそのまま信じない
...
- **作るのは `openImgView(list, i)` の1箇所だけ**（`[{url,name}]` の配列と番号を渡す）。
  ここを直せば**どの画面でも同じ見え方**になる
  - > ⚠️ 人材が送ってきた在留カード・資格証は、サムネイルのままでは
    > **文字が読めず中身を確かめられなかった**（実際に言われた）
- できること＝ 拡大・縮小（ボタン／ホイール／`+` `-`）／90度まわす／ドラッグで動かす／
  ‹ › と ← → で切り替え／`⤢` で元に戻す／🔗 ドライブで開く／✕・Esc・背景クリックで閉じる
- ⚠️ **z-index は `IMG_VIEW_Z`(100200)**＝ 外部フォーム（`#candFormOverlay` は 99998）より**上**にする
  （下だと押しても出てこない）
- ⚠️ **90度・270度にまわしたときは縦横の上限を入れ替える**（`_imgViewRender()` の中）。
  入れ替えないと、横長の書類をまわした瞬間に**画面からはみ出す**（実際にはみ出した）
- ⚠️ **ドラッグは `pointer` イベントで書く**（マウスも指も同じ処理。タブレットでも動く）
- 呼んでいるのは3か所＝ 人材資料保存（`openResumeDocView(key, i)`）／証明写真／求人票の写真
- 📄 **書類のサムネイルは `object-fit:contain`**＝切り取らずに全体を出す
  （`cover` だと在留カードの端が切れて確かめられない）
...
- **記録が入っているかの判定は `_meetingSchedDone()` の1箇所**
  ...
  - ⏰ **記入待ちの印は「📅 予定」の行だけでなく「記録の行」にも出す**＝
    **見るのは `renderMeetingList` の中の `todoIds`（＝バナーの件数 `todo` と同じもの）の1箇所だけ**。
    記録の行には「［✏️ 編集］から【✅ 対応結果】を入れると消えます」の1行も出す
    - > ⚠️ **過去の事故**: 印が「📅 予定」の行にしか出ていなかったので、
      > **相談内容を書いた記録があるのに件数だけ残り**、「記入したのにアラートが消えない」と言われた
      > （記録がある予定は 📅 予定 の行を出さないため、どれが残っているのかも見えなかった）
  - 🗑 **一覧の「📅 予定」の行からも予定を消せる**（［🗑 予定を削除］＝ `deleteSchedule()` の1箇所を通す）。
    ⚠️ 入れ間違えた予定をここから消せないと、**⏰ 記入待ちが消せないまま残る**（実際に残った）
...
- 🟢🟡🔴 **面談後の人材の状態は `company_meetings.worker_status`**（空＝未設定／`ok` 問題なし／`watch` 要注意／`problem` 問題あり）。
  **ラベル・色・名前の色・重さ（`r`）は `MEETING_STATUSES` の1箇所だけ**（読むのは `_meetingCondOf()`）
  - 入れるのは記入フォームの【記録内容】のいちばん上のボタン（`_meetingPickCond` → 隠しの `#mf_cond`）。
    ⚠️ **［▶ 次の人材を記録する］には引き継がない**（人ごとに違うため。`_meetingCarry` に入れない）
  - 人材の行・フォルダの 🔴🟡 の人数は **その入れ物の中でいちばん悪い状態**（同じなら新しいほう）＝ `_meetingWorstCond()` の1箇所
  - 一覧の上の「🔴 問題あり（最新の面談）」は**人材ごとのいちばん新しい記録の状態**で数える（数え方が違うので混ぜない）
  - 絞り込みのチップ `cond`＝要注意以上（`r >= 2`）。検索も状態の名前で引っかかる
...
### 📄 申請記録（1申請＝1行 ／ `visa_applications`）

- ⚠️ **1行＝1申請。人材ではなく「申請」を1件ずつ積み上げる**
  - > ⚠️ **なぜ作り直したか**: 以前は `workers` の `apply_*` の**1組しか持てず**、
    > 更新するたびに上書きされて**前の申請の記録が消えていた**。
    > チームは2019年からスプレッドシートで「1申請＝1行」で積み上げていて、
    > **「申請ごとに時系列で出るからやりやすい」＝好みではなくデータの持ち方の差**だった。
    > 実際 `apply_record_edits` は711行あって中身が入っていたのは24行だけ（3%）＝ほぼ使われていなかった
- **人材とは `worker_id` で結び付く**（人材情報・チャット・アラートと自動でつながる）
- 🔁 **いちばん新しい1件を `workers` に書き戻すのは `_vaSyncToWorker()` の1箇所だけ**＝
  `apply_date` / `apply_type` / `apply_status` / `apply_number`。
  **これまでの画面・更新リマインド・アラートは何も変えずにそのまま動く**
  - ⚠️ **書き戻すのはその人材でいちばん新しい申請日の1件のときだけ**（古い申請を直しても人材は動かない）
  - 🪪 ステータスが **発行完了・入国済み**になったときだけ、在留期限・特例期間・在留カード番号も人材へ入れる
    （＝ 更新リマインドが自動で消える）
- **選択肢は一かたまりの1箇所だけ**

  | const | 中身 |
  |---|---|
  | `VA_TYPES` | 認定／変更／更新／特活／特活→特定技能／特活更新／2号申請／2号特活（**8種**）。`wt` が `workers.apply_type`（認定/変更/更新）への読み替え |
  | `VA_STATUSES` | ⚠️ **`workers.apply_status` とまったく同じ言葉**（読み替えを持たない）。シートの言い方は `VA_STATUS_ALIAS` で取り込みのときだけ直す |
  | `VA_SITUATIONS` / `VA_RECEIVE` / `VA_CHAT` / `VA_MLIT` | 状況／受け取り方法／チャット／国交省 |
  | `VA_DONE_SITUATIONS` ＋ `VA_CLOSED_STATUSES` | ✅ 完了の枠に移る条件（**判定は `_vaBucket(r)` の1箇所**） |
  | `VA_STEPS` | 🖊️ 日付＋担当者のスタンプ（預り証 stock共有／預かり証 企業へ／在カパ 依頼連絡） |

- **列は `VA_COLS_DEF()` の1箇所だけ**（`k`／`l` 見出し／`w` 幅／`v` 表示／`s` くらべる値／
  `edit` その場で直せる種類／`step` スタンプ）。**列を足せば見出しの並べ替えもそのまま付いてくる**
- 🖊️ **スタンプは押すだけで「今日＋自分の名前」**（`vaStamp()`。もう一度押すと取り消し）＝
  シートの「4/23新宅」と同じ。手で打たせない
- 💾 **その場で直したら自動保存**（`VA_AUTOSAVE_MS`(1200)ms 後）。
  **書き込むのは `saveVaRow()` の1箇所だけ**で、未保存ぶんは `_vaDirty`（id → 直した中身）。
  画面を移るときは `flushVaSaves()` が先に書き込む（**確認は出さない**＝自動保存なので）
- 🗓 **特例期間＝在留期限＋`VA_TOKUREI_MONTHS`(2)ヶ月。計算は `vaTokureiOf()` の1箇所だけ**
- ➕ **記入フォームの項目は `VA_FORM_FIELDS` の1箇所だけ**。
  **人材を選ぶと空いている欄だけ自動で入る**（`vaFormPickWorker()` の1箇所＝
  企業・企業番号・国籍・業種・在留期限・特例期間・企業担当（メイン）・書類担当（サブ））
  - ⚠️ **新規で作ったら `_vaEditId` に id を覚える**（続けて保存したときに二重に登録しないため）
- 🏢👤 **人材を選ぶ前に企業を選ぶ。選択肢を作るのは `_vaCoOptionsHtml()` / `_vaWkOptionsHtml(coId)` の2つだけ**
  （いま選んでいる企業と検索の文字は **`_vaPick`（`co` / `coQ` / `wkQ`）の1箇所**に持つ）
  - 企業を選び直すと人材はその企業のぶんだけになり、選択と検索の文字は外す（`onVaCompanyChange()`）
  - **企業を選ばずに人材を先に選んだときは、企業の欄もその人材の企業に合わせる**
    （`vaFormPickWorker()` の中の1箇所。一覧に無ければ検索を解除してから入れ直す）
  - 🔍 **検索は `<select>` の中身だけを入れ替える**（`setVaCoSearch()` / `setVaWkSearch()`）。
    ⚠️ 検索欄そのものを作り直さない＝**1文字入れただけでフォーカスが外れて続けて打てない**
    （求人の【👥 候補者リスト】の検索と同じ考え方）
  - ⚠️ **`allCompanies` は起動直後はまだ空**のことがある。`openVaForm()` は
    `ensureCompaniesLoaded()` を待ってから企業の欄だけ作り直す
- 上のタブは3つ＝ ⏳ 進行中 ／ ✅ 完了 ／ 🗂 すべて（`_vaUI.bucket`）。
  **既定は進行中だけ**＝ 終わった申請が一覧から消えるので「いま処理するもの」だけが並ぶ
  （以前は申請ステータスが「申請予定なし」以外の人材を全部出していて209件になり、ずっと増え続けていた）
- 👤 **人材モーダルの「受け取り方法／チャット／状況」は、その人材のいちばん新しい申請を見る**
  （`_syncWorkerApplyToForm()` / `_saveWorkerApplyFields()`。`_vaWorkerLatest` の1件だけを書き替える）
- 📥 **シートの「KMT(新)申請記録」タブ 318件は `source='sheet_import'` で取り込み済み**
  （2026/02〜2026/09/11。氏名で人材と突き合わせ＝316件が紐付き）
  - ⚠️ **Google Drive の `read_file_content`（テキスト読み取り）はタブの途中で打ち切られる。**
    このタブは実際 319行あるのに **136行しか返らず**、2026/04〜09 のぶんがまるごと抜けていた
    （件数が変わらないので気づけなかった。実際に起きた）。
    **シートを全部読むときは `download_file_content` で xlsx を落として openpyxl で読むこと**
  - ⚠️ **氏名は人材側にニックネーム・【２号】が付いている**（`PANMEE NAWIN（ナーウィン）`）。
    突き合わせは **カッコ以降を落とし・空白を無視し・大文字にそろえてから**、
    **1人に決まるときだけ**紐付ける（同姓同名が実際にいる）
  - ⚠️ 同じ申請番号に2名入っていることがある（**同時申請**。`東オンCV26015499`）ので、
    一意は `(apply_number, worker_name)` にしてある。**`apply_number` 単独で一意にしない**
...
- 📣 **実際に送る宛先を作るのは `_visaFeeChatTargets(co)` の1箇所だけ**
  （本文の `@…`＝`tokens` も、通知の宛先＝`mentions` も、送る前の確認も必ずここから作る）
  - ⚠️ **送る人自身がメイン／サブ担当のときは、自分あてのメンションを @全員 に変える**
    （自分で書いて送った内容が自分に🔴で返ってきても意味がないため。実際に言われた）。
    **ほかの担当あてのメンションはそのまま残す**
  - 名前くらべは **`_renSameName()`**（空白を無視して前方一致でも見る）＝
    担当欄が「蛭田」で名簿が「蛭田　裕紀」でも同じ人とみなす
  - `mentions` には **@全員 のぶんを全員の名前に展開して入れる**（`_renPostStatusChat` と同じ考え方。
    `is_active=false` の人と自分は入れない）。本文に名前が書いてある担当は未読集計の①で🔴自分あて、
    それ以外は②で🔵全体あてになる
  - ⚠️ **本文には `tokens`、`mentions` には展開したほうを入れる**（取り違えると
    本文に無い名前が🔴になる・@全員 なのにだれにも届かない）
...
- ⏳ **期限カードを赤く点滅させる条件は `ALERT_CARD_URGENT`（カードのキー → 判定 → 文章）の1箇所だけ**。
  ...
  |---|---|---|
  | `a0` 特例期間中 | `_tokureiUrgent()` | **新しい在留カードをまだ受け取っていない**（`TOKUREI_URGENT_DONE`＝発行完了・入国済み **以外**）のまま、特例期間の残りが `TOKUREI_URGENT_DAYS`(15)日以下 |
  ...
  - 枠に出す文章は **`_tokureiUrgentText()` / `_visaPrepUrgentText()` の1箇所ずつ**
    ...
    点滅は既存の `.due-over`
    - ⚠️ **申請ステータスの「当たり」を並べて判定しないこと**（`['審査中','審査完了']` のように書かない）。
      > ⚠️ **過去の事故**: 審査中・審査完了のときだけ点滅させていたので、審査が終わって
      > **発行待ち** に変えた瞬間に点滅が消えた（特例期間 あと6日 の人材が静かな枠のまま並んでいた）
  - **一覧（`toggleAlertList`）も同じ `_cardUrgent()` を見る**ので、件数と中身が必ず一致する。
    対象の人材は先頭に赤枠で並べる
  - 📐 **4つのカードの一覧は「残りがいちばん短い人」から出す。並べるのは `renderWorkerAlerts` の1箇所だけ**
    （`_cardLeftDays` ＝ 特例期間中は `tokureiDays` ／ほかは `days`）。
    `toggleAlertList` は**先頭に寄せるだけ**（`sort` は安定なので並びは崩れない）。
    > ⚠️ **過去の事故**: 並べていなかったので、一覧が登録順のまま
    > 「あと52日 → あと6日 → あと30日」とバラバラに出ていた（実際に出ていた）
...
- 人材詳細（`#workerModal`）の**見出しの右**に、その人材のKMTドライブを開くボタンを出す
  （【4. ドライブリンク】まで下げないと開けなかったため）
- **置き場所はモーダル共通の `#<modalId>HeadExtra` の1箇所**（`ensureModals` がヘッダーに用意する）。
  ⚠️ **モーダルは使い回すので `showGenModal` が開くたびに空にする**＝
  入れ直すのは開いた側（人材は `renderWorkerHeadDrive()`）。ほかのモーダルに残らない
- **URLを読むのは `_wkKmtDriveUrl()` の1箇所だけ**＝ 入力欄（`wf_drive_url_kmt`）が開いていればその値、
  無ければ人材の行（`drive_url_kmt`）。開いたまま貼り替えても、そのまま開ける
  （欄の `oninput` で `renderWorkerHeadDrive()` を呼ぶので表示もすぐ変わる）
- ⚠️ **欄には URL 以外の文字（`なし` `-` など）が入っている**ので、開けるかどうかは
  **`_driveFolderId()`**（チャットの添付と同じ見方）＋ `http(s)` で始まるかで見る。
  開けないときは**「📁 KMTドライブ 未設定」**と出し、押すと基本情報タブの
  その欄まで移動して光らせる（`wfStatusGo()` を共用）
- 押したときの動きは **`openWorkerKmtDrive()` の1箇所だけ**
...
- 特定技能1号の途中で本帰国し、あとから在留資格認定で入り直した場合、
  **母国にいた期間は通算期間に入れない**。入れ方は2つあり、**どちらでも結果は同じ**
  1. **🛫 母国滞在の行を足す** … 在留資格に `TOKUTEI_ABSENT`（`（母国滞在・不在）`）を選び、
     日付に**帰国した日**を入れる。`_isTokuteiCountVisa()` が false なので通算に足されない
...
##### 🧮 どの行を通算に入れるか（`_isTokuteiCountVisa` / `_tkCounted`）

- **通算に入れる在留資格は `_isTokuteiCountVisa()` の1箇所だけ**＝ **特定技能1号・2号 ＋ 特定活動**。
  ⚠️ **特定活動（特定技能への移行準備）も入管庁は通算に含める**ので入れる
  （画面の注意書きどおり、ここに入力するのは会社名の記載がある指定書のぶんだけ）
  - ⚠️ **`_isTokuteiVisa()`（特定技能だけ）と混ぜないこと。** あちらは「特定技能の行か」を
    見るところ（`認定` の警告など）で使う。通算に入れるかは必ず `_isTokuteiCountVisa()` を見る
- **行ごとの最終判定は `_tkCounted()` の1箇所**＝ 🍼 休業の行だけは「含む／除外」（`r.count`）で決まり、
  **既定は「含む」**。それ以外の行は `_isTokuteiCountVisa()` を見る
  - 🍼 **産前産後休業・育児休業は通算に入る**（入管に申立書を出して許可された行だけ「除外」にする）
- ⚠️ **起点（`startDate`）より前の行はどこにも数えない**（通算にも空白にも）。
  技能実習のころの古い特定活動が残っている人材がいて、数えると**その空白ぶん満了日が何年も後ろへ飛ぶ**
  （実際に満了日が 2045年 と出ていた人材がいた）。`calcTokuteiSummary` の中の
  `if (from < startDate) return;` の1箇所
  - そのため **起点は「初回」チェックの行が最優先**（チェックが無いときだけ通算に入る最初の行）。
    ⚠️ 「通算に入る一番早い行」を起点にすると、古い特定活動に引っぱられて**満了日が2012年になる人材が出た**ので採らない

> ⚠️ **入管の実際の許可とはまだ差がある（未解決）。**
> 在留期限が短く切られている（＝入管が5年上限に当てた）46名で突き合わせたところ、
> **入管は「初回＋5年（暦）」で切っていて、空白日数ぶん後ろにずらしていない**。
> 空白0日の人はシステムと入管が −1〜−24日でほぼ一致し、空白がある人だけシステムが空白ぶん後ろにずれる。
> 残る 0〜30日 のずれは、**入れてある「指定書発行日」が入管の在留期間の開始日より数日〜1ヶ月早い**
> ためと思われる（原本で確認待ち）。ずらしをやめるかどうかは**未決**。
...
#### ✅ 精査のチェックは2つ（担当精査 ／ 指定書チェック）

- **一覧は `TK_VERIFY_CHECKS` の1箇所だけ**（列・絞り込みのチェック・件数チップが同時に追従する）

  | キー | 列 | 画面の名前 | 誰が押すか |
  |---|---|---|---|
  | `hv` | `history_verified` | 経歴内容を担当が精査済み | その人材の担当 |
  | `sv` | `shitei_verified` | 📄 指定書チェック済み | **別の担当**（指定書の原本を見る人） |

- どちらも **チェックすると日時（`*_at`）とユーザー名（`*_by`）が入る**（`onHistoryVerifiedChange` /
  `onShiteiVerifiedChange`。保存で確定）。置き場は 10番の計算結果の下に**横に2つ並べる**
- 🚫 **指定書チェックはアラートに出さない**（別の担当が確認するものなので）＝
  `missingRequiredFields()` にも `computeGlobalWorkerAlerts()` にも**足さないこと**。
  見るのは 📅 通算期間リストの絞り込みだけ
- 軽いSELECTでも読めるよう **`WORKER_ALERT_SELECT_COLS` に列を入れてある**

### 📅 通算期間リスト（外国人材の4つめのタブ）

- 📋 一覧／📅 更新状況／📄 申請記録 のとなり。**中身は人材一覧と同じ `workers` の行**
  （別テーブルを作らない）。**通算期間の長い順**にならべて、精査の進みぐあいをまとめて見る画面
- **通算期間を出すのは `calcTokuteiSummary()` の1箇所**（10番の計算とまったく同じ）。
  5年満了日も `worker5yExpiry()` を通すので、**人材一覧・更新リストと必ず同じ値**になる
- **列は `TK_LIST_COLS_DEF()` の1箇所だけ**（`k`＝並べ替えのキー／`l`＝見出し／`v`＝表示／`s`＝くらべる値）。
  **列を足せば見出しを押したときの並べ替えもそのまま付いてくる**
  - 並べ替えは `setTkListSort()`（1回目 A→Z ／ もう一度で Z→A）。
    **空欄は向きにかかわらず必ず最後**（空が上に来ると何も見えないページができる）
  - 既定は **通算期間の長い順**（`_tkList.sort` の初期値）＝ 5年満了に近い人を先に出す
- **絞り込みは `_tkListFilter()` の1箇所だけ**＝ 検索（管理番号・氏名・国籍・企業・担当・在留資格・期限）／
  国籍／担当／**`TK_VERIFY_CHECKS` の 済み・未 のチェック4つ**。
  選択肢（国籍・担当）は**表に出しうる人材から作る**ので、いない値は出ない
- ⚠️ **この画面は `tokutei_history` と精査の列が要る**ので、軽いSELECTで上書きされていたら
  `_ensureTokuteiDataLoaded()` が取り直す（`_ensureRenewalDataLoaded()` と同じ作り）
- ⚠️ 検索は打つたびに描き直すので、**フォーカスとカーソル位置を戻してから返す**（`setTkListQ()`）
- 🧪 テスト用の人材（`_isTestWorker()`）は出さない
...
- 10番は3つに分けて見出しを付けてある＝ **A. 特定技能１号の経歴**（特定技能１号と移行準備の特定活動＝就労先の行・計算結果・精査のチェック）／
  **B. 特定技能２号の経歴**（`tokutei2_history`）／**C. その他在留資格の経歴**（`past_employment`）
- **見出しを作るのは `_tkSubHead(letter, title, note, color, bg)` の1箇所だけ**（3つとも同じ形）
  - ⚠️ `.section-title` / `.form-label` にしない＝【基本情報】の項目検索（`_wfFindCollect()`）に拾われないように
  - ⚠️ この見出しは人材モーダルの**テンプレート文字列の中**にあるので、そこのHTMLコメントに**バッククォートを書かない**
    （文字列がそこで終わって壊れる。実際に壊れかけた）
...
- セクションの下に枠が2つ。**入力する項目は同じ**で、在留資格の欄が出るかどうかだけが違う
  ...
  | 🏢 過去の所属又は勤務先（特定技能以外） | `workers.past_employment` | あり（`PAST_EMP_VISA_OPTS`。先頭は技能実習１号） |
...
- 履歴書の保有資格は `candidates.licenses`（jsonb）。いまの形は
  ...
  **名前が空の項目も消さずに残す**（履歴書に空の枠として出て、手書きで足せるようにするため）。
  🏆 **履歴書では「無」の資格を資格名ごと薄くする**＝ ✓ 有 のものだけが黒で残り、
  持っている資格がひと目で分かる。**付けるのは `.resume-lic-item.off` の1箇所**
  （`li.name && !li.has` の行だけ。空の枠は今までどおり）。
  ⚠️ **CSSは画面用と書類用の2つあるので必ず両方そろえること**。
  ⚠️ **このCSSは `buildResumeHTML` のテンプレート文字列の中にある**＝
  コメントにバッククォートを書くと、そこで文字列が終わって壊れる（実際に壊れた）
...
- **入口は `sendKmtDecisionAll()` の1箇所だけ**（画面は［📤 決定報告を送信（全部まとめて）］の1つ）。
  **押すと先に「何がどこに反映されるか」の確認が出て、［✅ OK・送信する］のときだけ進む**
  - **確認の文面を作るのは `_kmtDecisionPlanHtml(data)` の1箇所だけ**（`askChoice()` で出す）。
    ⚠️ **実際に走る処理と同じ値・同じ条件から作ること**（画面と結果が食い違わないように）＝
    売上に入れない理由（**`_drSaleSkipReason()`**）も、請求条件のモード（**`_drBillingMode()`**）も、
    金額も請求月も、`submitDecisionReport` / `saveDecisionInitialFeeToSales` と同じ見方をする
  - ⚠️ **企業名が `companies.name` に無いときは赤で知らせる**（誤字のまま送ると売上が企業に紐づかない）。
    人材名が空のときも③⑤が行われないことを出す
  - やることは5つで、**順番を変えないこと**
  ...
  | ③ | 「外国人材に追加しますか？」→ 人材を登録／更新 | ①の中（`saveDecisionAsWorker`。聞き方は `_drWorkerConfirmText()`） |
  | ④ | 売上管理に「初期費用」／「申請費用」を登録 | ①の中（`saveDecisionInitialFeeToSales`） |
...
- 📸 **送信は押した時点の控え（`_drKmtSnapshot()`）だけを見る。画面を読み直さない**
  （`data`＝`getDecisionData()` ／ `text`＝`buildKmtReportText()` ／ `workerId`＝⚡既存人材 ／ `joinDate`）。
  `submitDecisionReport(snap)` / `copyKmtReport(snap)` / `_ensureWorkerFromDecision(snap)` に同じものを渡す
  - > ⚠️ **過去の事故**: 以前は送信のあとで画面を読み直していた。スプレッドシート送信（GAS）は時間がかかり、
    > 読み込み中の覆いは `LOADING_MAX_MS`(20秒) で消えるので、**そのあいだに画面を移ると
    > 見出しだけの空の文面がコピーされ、スプレッドシートにも入らなかった**（2026/09/24 に実際に起きた）
  - ⚠️ **送れなかったときはトーストだけで終わらせない**＝ `_drKmtSendFailed(snap)` が理由・文面を出したままにして、
    ［🔁 もう一度送信する］でやり直せる。返事が返ってこなかっただけ（`snap.noReply`＝時間切れ・`_chatDriveNoReply()`）のときは
    **届いていることがある**ので、2件にならないよう先にスプレッドシートを確かめてもらう
  - GAS を待つのは `DR_GAS_TIMEOUT_MS`(60秒) まで。二重に押しても `_drKmtSending` が止める
  - ⚠️ **入社希望日は「許可おり次第」という文字のことがある**＝ 人材の `joined_date`（date の列）には
    **YYYY-MM-DD のときだけ**入れる（そのまま送ると人材の自動登録がまるごと落ちていた）
- 🚫 **売上に入れない理由を決めるのは `_drSaleSkipReason()` の1箇所だけ**
  （トーストで「◯◯のため登録していません」と知らせる）＝
  フルサポートプラン ／ 初期費用のとき（`init`）の【B区分】更新・1-6修正。
  ⚠️ **特定技能2号はここでは止めない**（「申請費用」のほうに回るため。もとは止めていた）
...
- 🏢 **決定報告の企業名 → 所属機関を引くのは `_drCompanyOf(name)` の1箇所だけ**
  （送信前の確認・`_drFindExistingWorker`・`saveDecisionAsWorker`・`saveDecisionInitialFeeToSales`）。
  ①ぴったり同じ名前 ②無ければ `_drCoNorm()` でならして**1社だけに決まるとき**だけ拾う
  （「（株）英光」→「株式会社英光」）。確認の画面には「◯◯ として登録」と出す
  - ⚠️ **誤字（「栄光」と「英光」）は拾わない**＝ 確認の画面に赤で「所属機関に見つかりません」と出る
- 💰 **`saveDecisionInitialFeeToSales()` は成否を返す**。落ちたときは `alert` で残す（トーストだけだと消えて気づけない）。
  **同じ人材・同じ項目・同じ企業がもう入っていれば作らない**（送り直しで2件にしない）
- > ⚠️ **過去の事故（2026/09/14 受領の（株）英光 2名）**: 決定報告をシステムから送信せず、
  > 依頼スプレッドシートに**手で書き込んで**いた（コメント欄が「確認中」だけ＝アプリが書く形ではない）。
  > そのため `decision_report_log` も売上も0件で、**9月の売上に出なかった**。
  > 2026/09/25 に 9月の「初期費用（未定）」として売上に後から入れた（金額は決定報告に無かったため空）

#### 💴 請求条件は【A区分】希望在留資格で中身が変わる（init ／ full ／ apply）

- **どのモードかを決めるのは `_drBillingMode()` の1箇所だけ**

  | 希望在留資格 | モード | 出す項目 | 売上 |
  |---|---|---|---|
  | 特定技能1号・特定活動・未選択 | `init` | 初期費用／請求タイミング（決定時・配属後）／支援委託費／請求書 ＝ **従来どおり** | 初期費用（更新・1-6修正は入れない） |
  | 特定技能2号 ＋［フルサポートプラン（申請費用なし）］にチェック | `full` | 支援委託費（変更なし／あり）と収入印紙代・メモ**だけ** | **入れない** |
  | 特定技能2号（チェックなし）・技人国・家族滞在 | `apply` | **申請費用**・請求先（企業／人材）・請求タイミング（決定時・**許可後**）・請求書（送付方法／送付先／担当＝既定 `DR_INVOICE_STAFF_APPLY`） | 申請費用（**申請種別を問わず入れる**） |

- 欄の出し入れは **`updateDrBillingFields()` の1箇所**（`dr_wrap_*` の id を出し入れするだけ）。
  請求書の送付先の欄（メール／その他）は **`updateDrInvoiceMethod()` の1箇所**で、**`apply` のときだけ**出す
- **■請求条件の行を作るのは `_drBillingLines(d, withInvoice)` の1箇所だけ**＝
  文面（`buildKmtReportText`）も大房へ送るコメント（`getDecisionData` の `comment`）も同じものを通る。
  ⚠️ **同じ行を2か所で組み立てないこと**
- ⚠️ **出さないモードの値は `getDecisionData()` で空にして渡す**
  （大房のシートに前のモードの金額が残らないようにするため）
- 💰 **申請費用は大房行政書士法人に入るお金なので、KMTの売上として数えない。
  対象は `SALE_ITEMS_OFF_TOTAL` の1箇所だけ**（下の「売上管理 月別シートの合計行」も参照）＝
  ［全項目］には出さず、合計・売上内訳にも足さない。［項目：申請費用］を選んだときだけ見える
  - ⚠️ 内・申請費用（`app_fee`＝5万円）は**初期費用のときだけ**。申請費用の行には付けない
  - 請求タイミングが **配属後請求（その他）／許可後請求**のときは請求月を空にして📦配属後請求ページへ

#### 👤 ③「外国人材に追加しますか？」＝ 新規登録か既存人材の更新か

- **決めるのは「氏名＋企業名がそろって一致する人材がいるか」だけ。⚠️ 申請種別では決めない**。
  探すのは **`_drFindExistingWorker(data)` の1箇所だけ**（`{worker, why}` を返す）
  - ⚠️ **【認定】でも、同じ企業に同じ氏名の人材がいればその人の更新**（同じ人を2件作らない）
  - ⚠️ **【変更・更新】でも、その企業に同じ氏名がいなければ新規登録**
    （他社から移ってきた人など、まだKMTに登録されていないことがある）
  - ⚠️ **氏名が同じでも企業が違えば別人**（＝新規登録）。同姓同名が実際にいるため
  - ⚠️ 企業名が `companies.name` に無いときは照合できないので新規登録。
    **`why` に理由を持たせて確認ダイアログに出す**（企業名の誤字がそのまま新しい人材になるため）
  - > ⚠️ **過去の事故**: 以前は申請種別だけで決めていた（`_drWorkerIsNew()`）。
    > **認定なら重複を見ずに必ず新規登録**していたので、候補者から人材登録ずみの人で
    > **同じ人が2件できる**状態だった。逆に変更・更新で未登録の人は、
    > 「既存人材の更新」と聞いておきながら見つからず、もう一度たずね直していた
- **確認ダイアログの文面は `_drWorkerConfirmText(data, hit)` の1箇所だけ**。
  ⚠️ **呼ぶ側（`submitDecisionReport`）が先に探して、その結果を文面にも
  `saveDecisionAsWorker(data, hit)` にもそのまま渡す**＝ 聞いた内容と実際の動きが必ず一致し、二重に引かない
  - ⚠️ **「追加しますか？」とだけ聞かないこと。** 既存人材のときに**新しく増える**と誤解される（実際に言われた）。
...
- 🖱️ **名前以外の項目を押すと、その記録の編集画面が開いて押した欄まで動いて光る**
  （記録簿の一覧・支援実施体制管理簿の「項目ごとの管理簿リスト」・外国人材の【その他支援】タブ）
  - **入口は `regCellEdit(no, id, workerId, companyId, key)` の1箇所だけ**。セルに付ける属性は
    **`_regCellHitAttr()` の1箇所**（`class="reg-cell-hit"`。ホバーの色はこのクラスのCSS1箇所）
  - 記録がある行＝その記録／**未記入の行＝その人材・所属機関を入れた新規の記入フォーム**（［＋ 記入］と同じ）
  - `openRegTaskForm` の5つめの引数 `focusKey` を渡すと、最後に **`_regFocusField()`** が
    タブ（`sec`）を切り替えてその欄まで動かす
  - 列のキーと欄のキーが違うものは **`REG_CELL_FIELD` の1箇所だけ**
    （`_s0`→`sessions` ／ `_r0`・`_total`→`rows` ／ `_members`→`members`）。
    **`listCols` に足した列が欄のキーと同じなら何も書かなくてよい**
  - ⚠️ **文字を選んでいる間は開かない**（一覧はコピーして使うため。労働局の管理簿と同じ）
  - ⚠️ 名前の列（`_worker` / `_company`）には付けない＝今までどおり人材・所属機関が開く
...
#### 📑 2. 支援委託契約書（5-10号）の【契約書の状況】

記入フォームは2つの**タブ**に分かれている。分けるのは **欄の定義に書く `sec` の1箇所だけ**
（`sec` を書いた欄から次のかたまりが始まる。ほかの様式でも同じように使える）。
**`sec` が2つ以上あるときだけタブになる**（1つも無い様式は今までどおり1枚のまま）。
切り替えは `switchRegFormSec(i)` の1箇所。
⚠️ **開いていないタブの欄も DOM には残す**（`display:none`）＝ `_regCollect()` が `rgf_*` を
そのまま読むので、**どちらのタブの内容も 💾 保存で一緒に保存される**（空で上書きされない）。

| タブ | 中身 |
|---|---|
| 📄 書類作成 | これまでの欄（甲乙丙・委託料・契約期間・別紙 …） |
| 📑 契約書の状況 | 企業・人材書類チェックリストから入れる5項目＋メモ |

- **項目は `REG_CONTRACT_ITEMS` の1箇所だけ**（記入フォームの欄・保存・一覧の列が同時に追従する）
- 1項目＝**2つの値**＝ 有／無し（`REG_CONTRACT_STATES`）と ☑ ドライブ格納有
  （キーは `REG_CONTRACT_DRV(k)`＝`<k>_drv`）。例: 新しい委任契約書（有）☑
- ⚠️ **`scope` で持ち場所が変わる。混ぜないこと**
...
#### 🔍 一覧の並べ替え・絞り込み・経理チェック済みを隠す（立替管理・カード決済報告で共通）

- **2つの画面の違いは `PAY_LIST_CFG` の1箇所だけ**（日付の列・支払方法の列・選択欄のid・読み直す関数）。
  状態は **`_payListUI` の1箇所**（`dir`＝利用日の向き／`hideChecked`＝経理チェック済みを隠すか）
- ⚠️ **一覧はサーバー側でページを分けている**（`sbFetchPage`）ので、**絞り込み・並べ替えは問い合わせに入れる**＝
  **作るのは `_payListQuery(table)` の1箇所だけ**（担当者 `staff` ／ 支払方法 `payment_method`・利用カード `card` ／
  `acct_checked=not.is.true` ／ `order=<日付>.<向き>.nullslast,created_at.desc`）。
  画面に出ている20件だけを並べ替えると、ページをまたいだ順番がおかしくなる
- 利用日の向きは［📅 利用日 新しい順／古い順］のボタンと、表の「利用日」の見出しの**どちらでも**切り替わる（`togglePayListSort`）
- 担当者・支払方法の選択肢は **DBに入っている値から作る**（`_payListFillOpts`。一覧を読むたびに取り直す）。
  支払方法は決まった一覧（`EXPENSE_PAY_METHODS` / `CARD_LIST`）を先に並べ、一覧に無い昔の値（「クレカ」など）を後ろに足す
- 隠しているときに経理チェックをその場で付けたら、描き直しで消える（`_payListRows`）
- ［✕］（絞り込みを消す）は、隠す設定も一緒に戻す。並び順はそのまま
...
- **上のサマリ（総合計カード・📊 売上内訳）はその月の全体**（絞り込んでも変わらない）。
  ここを `rows` に変えると「その月の売上」が分からなくなるので変えないこと
- 💜 **KMTの売上として数えない項目は `SALE_ITEMS_OFF_TOTAL` の1箇所だけ**（いまは大房行政書士法人に入る `申請費用`）。
  ⚠️ **表・合計・売上内訳・総合計は `allSales` ではなく `base`**（＝`SALE_ITEMS_OFF_TOTAL` を除いたもの）**から数えること**。
  ［全項目］にも出さず、［項目：申請費用］を選んだときだけ見える
  - ［項目］の選択肢と売上の編集画面の［項目名］は、**どちらも `SALE_ITEMS_OFF_TOTAL` から
    いちばん下に薄い字で**出す（定義を2つ持たない）。
    ⚠️ **編集画面の選択肢に足し忘れると、その行を開いて保存したとき先頭の「初期費用」に化ける**（実際に化けた）
- 絞り込みは5つ（**状況（未作成／未請求／未入金）** ／ 項目 ／ 支払方法 ／ 請求方法 ／ 請求法人）。
- 🏷️ **【項目】はいくつでも選べる。選んでいる項目は `saleItemSel` の1箇所だけ**（空＝全項目）。
  選択肢は `SALE_ITEM_OPTS` ＋ `SALE_ITEMS_OFF_TOTAL`（いちばん下に薄い字）、パネルを作るのは `saleItemPickHtml()` の1箇所。
  ⚠️ パネルは月の表（`saleContentArea`）の外に置く＝表を描き直しても開いたままで続けて選べる
...
- 📊 **サマリの「新規決定数」（初期費用）と「紹介人数」（人材紹介料）は、行数ではなく数量の合計**。
  数えるのは `renderSaleMonthTable` の中の **`_qtySum(項目名)` の1箇所だけ**（数量が未入力の行は1件）
- 📅 **請求月は `saveSale` で `_saleYm()` の1箇所を通して「YYYY-MM」にそろえる**（読めなければ保存しない）。
  ⚠️ 月の表は `billing_month=eq.YYYY-MM` で引くので、「2026－09」（全角）「2026/9/24」「2026・9・24」の
  ように入ると**どの月にも出ない**（2026/09/25 に実際に3件あった）
- 📅 **売上の編集画面の【請求月】は3択。決めるのは `_saleBillingMonthHtml()` / `_saleBillingMonthValue()` の1組だけ**
  ＝ **📅 今月（既定）**／📦 配属後請求（`null`）／✏️ 手入力（`<input type="month">`。選んだときは今月が入る）。
  既存の行は 今月と同じ→今月・空→配属後請求・それ以外→手入力 で開く（`_saleBillingMonthKind()`）。
  手入力が空・読めないときは `undefined` を返し、`saveSale` が止める
