...
- 請求月の **翌々月4日**（`KOZA_DAY` / `KOZA_MONTHS_AHEAD`）。
  その日が **土日・祝日・年末年始（12/31〜1/3＝`BANK_CLOSED_MMDD`）** にあたるときは **翌営業日**
  （例 2026年8月末請求 → 10月4日は日曜なので **10月5日(月)**）
- **日を決めるのは `kozaTransferInfo()` / `kozaTransferDate()` の1箇所だけ**。
  請求書CSVのお支払期限（`_mfDueDate`）も、売上管理の【🏦 口座振替スケジュール】も同じものを見る
- **祝日は社内の「📅 年間カレンダー」（`company_calendars.holidays`）が正**。
  読むのは `loadBankHolidays()` の1箇所で、請求書CSVを開くときに取る。
  ⚠️ **その年度が未登録のときは土日と年末年始だけで計算する**ので、
  スケジュールの画面が⚠️で知らせる（`_kozaFyLoaded()`）。黙って違う日を出さないため
- 一覧は売上管理【🧾 請求情報】の［🏦 口座振替スケジュール］（`openKozaSchedule`）。
