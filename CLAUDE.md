...
- **一覧の氏名を押すとその人材の【基本情報】が開く**（`entLink()` の1箇所。
  所属機関ごとの様式なら所属機関の画面）。一覧に出す値は
...
- 出すのは **`WORKER_OTHER_TASKS` の1箇所**（日本語学習支援＝no.9／日本人との交流促進支援＝no.11／
  転職支援＝no.12。`label` が見出し、`no` が記録簿の番号）
...
- 除外者は2種類あるが、**まとめて `_fmIsExcluded()` の1箇所で判定する**。
  ...
  | 種類 | 実体 | 氏名の左の印 |
  |---|---|---|
  | ✕で手動で外した人 | `fee_excluded_workers` | ↩（押すと表に戻る） |
  | 退職済で直近1年に記録がない人 | 自動判定（DBに書かない） | 🗑（押せない） |
...
- **退職済で「直近1年（今月を含む12か月）」に何も入っていない人材**の判定は
  **`fmHasRecentRecord()` / `_fmRetiredNoRecord()` の1箇所だけ**
  ...
  - **自動除外に ↩ は出さない。** DBに書かないので ↩ で戻しても次に開くとまた消えるため。
    セルに何か入力すれば自動で表に戻る（`fmHasRecentRecord` が true になる）
- 人数の内訳は「除外者を表示」の横（`#feeExcludedCount`）に出す。
  **黙って消さない**（なぜ居ないのか分からなくなるため）
