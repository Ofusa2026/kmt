...
### 📅 所属機関の定期面談（`company_meetings`）

- 記録は **年度 × 期間（4〜6／7〜9／10〜12／1〜3）** で並べる。入口は所属機関詳細の【📅 定期面談】タブ
- 欄は 年度・期間・面談日・面談相手（企業担当者）・参加外国人材・**対象者の氏名**・**相談内容**・**対応結果**
...
- **対象者の選択肢を作るのは `meetingStaffOptionsHtml(cur)` の1箇所だけ**。並びは
...
- 一覧では相談内容と対応結果を **見出しを付けて分けて出す**（どちらが何か分かるように）。
  対象者は面談相手の隣に 🙋 のバッジで出す
- 相談記録書（5-4号）の「📥 所属機関の定期面談から取り込む」（`regImportMeetings`）は
  **相談内容（`memo`）を「相談内容」の列に入れる**。様式に対応結果の列は無いので取り込まない
