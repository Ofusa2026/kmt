...
- **画面に出す通し番号は `regTaskDispNo()` の1箇所**が並び順から作る（数えるのは `ledger` だけ。
  参考様式は記録簿の中に ↳ で出るので数えない）。見出しは `regTaskLabel()`（サイドバー・
  画面の見出し・`SCREENS` のタイトルが同時に追従する）
- ⚠️ 参考様式（`form`）は **その直前の記録簿にぶら下がる**（`regSupportFormsOf`）。
  記録簿を足すときは**様式の後ろに入れる**こと（間に割り込むと様式の親が変わる）
...
#### 📄 事前ガイダンスの確認書（5-9号 ／ no.4 ／ 表示は「3.」）

- **`kind:'ledger'` にしてある**（企業と人材の一覧＋書類生成を持つ独立した項目）。
  そのぶん 支援対象者管理簿の ↳ からは外れている
...
#### 🧾 支援経費収支記録簿（no.15 ／ 表示は「3.」→ 5-9号が入って「4.」）
