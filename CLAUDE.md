...
- **`showScreen` では「バナーを描く → 既読にする → バッジを更新」の順を変えないこと**
  （先に既読にすると新着の件数が0になる）
- 🔽 **どの画面も、新着があっても最初はたたんだ状態で出す**（見出しの「新着 ◯件」だけ。
  押すと開く）。**開くかどうかは `_updBarStartOpen()` の1箇所だけ**（常に false を返す）。
  ⚠️ 最初から開くと下のアラートが押し下げられて集中できない（実際に言われた）。
  以前は `UPDATE_BAR_MIN_SCREENS` で画面を選んでたたんでいたが（外国人材だけ）、全画面たたむことにした
...
| `hsp` | 🎓 高度人材の求人票（107項目・日英併記） | `JOB_SHEET_HSP_GROUPS` | `job_progress.sheet_ext.hsp` |
| `general` | 📋 一般の求人票（86項目） | `JOB_SHEET_GEN_GROUPS` | `job_progress.sheet_ext.general` |

- **並びは 特定技能 → 高度人材 → 一般**（`JOB_SHEET_KINDS` の順そのまま。選択欄・書類・一覧の印が追従する）
- 🎨 **種類ごとのテーマ色は `JOB_SHEET_KINDS` の `theme` の1箇所だけ**
  （`bg`＝見出しの帯／`fg`＝帯の文字／`line`＝タイトルの下線／`cell`＝項目名の背景／`border`＝枠線）。
  読むのは **`jobSheetTheme(kind)` の1箇所**。特定技能＝紺／高度人材＝薄い青／一般＝明るいグレー
- 書類の題名は `title`（＝「求人票」）＋ `title2`（種類名）の**2行**。特定技能は `title2` なしの1行
...
- ⚠️ **項目の `no` と キー（`k`＝`g<no>` / `h<no>`）は、入力済みの内容と結びついた内部の番号。
  振り直さないこと**（入力済みの内容が迷子になる）。足すときは使っていない番号を末尾に足す
- 🚫 **今後使わない項目は消さずに `off:true` を付ける**（入力済みの内容を残すため）。
  **ふるうのは `jobSheetGroups(kind, all)` の1箇所だけ**で、項目が1つも残らない見出しは落とす。
  `all` を渡すと**参考用（元の全項目）**になる
  - 📄 **参考用の様式は管理者だけ**（`openJobSheetRefDoc()` ／ 求人票作成の画面のボタン）。
    これまでの様式をそのままの内容で残してある（読むだけ・印刷はしない）
  - 📄 **【見本】ハローワークの「求人票（高卒）」の様式も管理者だけ**
    （`buildJobSheetSampleDoc()` / `openJobSheetSample()` の1箇所。読むだけ）。
    KMTの求人票とは別物で、ここを直しても実際の求人票には影響しない
    - ⚠️ **たて書きに `writing-mode` を使わない**（環境によって文字が重なって出る。実際に重なった）。
      **1文字ずつ改行する `_jsVert()` の1箇所**を通す
    - A4縦1枚に収めるための縮小率は **`JS_SAMPLE_ZOOM` の1箇所**
  - いま使う項目数は 📋 一般 78（元86）／🎓 高度人材 75（元107）。
    高度人材は**日英併記で1行が2段になる**ので、75項目でも**A4縦2枚**（1枚にすると読めない大きさになる）
  - **画面と書類に出す番号は `_jsExtNoMap(kind)` の1箇所**が並び順から作る通し番号。
    項目を外すと `no` に飛びが出るので、表に出す番号は分けてある
    （記録簿の `regTaskDispNo()` と同じ考え方）
...
- **書類を組み立てる入口は `buildJobSheetDoc(j)` の1箇所**（分類を見て振り分ける）。
  ...
  - 🎓📋 **高度人材・一般は特定技能の求人票と同じ「1ページ2列」**（`JS_COL_W` / `JS_COL_GAP` の1箇所）。
    左の列を上から使い、入りきらないぶんを右の列へ送る。**空欄の状態でA4縦1枚におさまる**
    - ⚠️ **ページ幅いっぱいに1行4項目ずつならべない**。項目名も記入欄も細切れになって
      「構造的に見にくい」と言われた（実際に言われた）。2列にすると1列の中は
      **項目名｜記入欄の横ならび**（`fCell()`）にでき、特定技能の求人票と同じ読み方になる
    - **項目の幅を決めるのは `_jsUnits(f)` の1箇所だけ**＝ **列の半分（6）か列いっぱい（12）**。
      文章を書く項目（`JS_LONG_RE` / textarea / `full`）と項目名が長いものは列いっぱい。
      項目ごとに決めたいときは定義に `w:` を書けばそれが優先される
    - 📐 **1ページ（2列）に入るときは左右の高さをそろえる**（左を天井まで詰めると
      右の列の下が大きく空く）。`JOB_SHEET_COL_PX` は**1列に入る高さ**
    - ⚠️ **2列は `<table>` で組む（grid にしない）**＝ Chrome の印刷は grid をページで分割できない
    - ⚠️ **行は必ず幅いっぱいにする**（セルは `flex:<units> 1 0%`。`width:%` にすると
      右側があまってギザギザに見える）
    - ⚠️ **枠線はセルが右と下だけを持つ**ので、**左端は行の入れ物**（`border-left`）、
      **見出しの帯の下**（`secBar` の `border`）で受け持つ。どちらかを外すと線が切れる（実際に切れた）
      - ⚠️ **見出しの途中で列が変わると、右の列の1行目だけ上の線が無くなる**（実際に切れた）。
        **列のいちばん上が「行」のときは `_jsTopLine()` の1箇所**で上の線を足す
    - 📐 **どのページも同じくらいの高さになるように割る**＝ `budget` は
      `min(JOB_SHEET_COL_PX, ceil(totalH / (2 × ページ数)) + 30)`。
      ⚠️ **均等割りで紙が1枚増えたら、元の詰め方（`JOB_SHEET_COL_PX`）に戻す**
      （見出しが列をまたげないぶんの取りこぼしで 4枚 → 5枚 になった）。
      割るのは `_jsPack(bud)` の1箇所で、**枚数を増やさないほうを必ず優先する**
    - 🇬🇧 **🎓 高度人材に英語を出すかどうかは `_flyerShowEn()` の1箇所だけ**
      （求人票作成の画面・種類の行の［🇬🇧 英語の項目名も入れる］。値は `_flyerShowEnOn`）。
      **既定は出さない**＝英語を出すと1行が2段になり、A4縦1枚におさまらない（出すと2枚になる）。
      チェック欄が無い画面（求人案件モーダルの求人票プレビューなど）でもこの値を見る
      - ⚠️ **チェックを外したら、書類の中の英語は1つ残らず消すこと。**
        項目名（`f.en`）だけ消していたので、**見出しの「Ⅰ．受付情報（紹介事業者が記入） / Intake」と
        いちばん上の「Job Order for Highly-Skilled Foreign Professionals」が英語のまま残っていた**
        （実際に残った）
      - **英語を落とすのは `_jsJaOnly(t)` の1箇所だけ**＝ `/`（半角・全角）で区切られたかたまりのうち、
        **英字があって・かなも漢字も無い**ものを落とす。見出しも上の1行も同じきまりで日本語だけになる。
        通すのは `buildJobSheetKindDoc` の中の `jaOnly()`（`secBar(g.sec)` と `def.sub` の2か所）
    - 🎨 **見た目は【見本】（ハローワークの様式）にそろえてある**＝ 番号は丸数字（`_jsSmpNo()`。求人票は黒・見本はピンク）、
      罫線は黒い細線（`JS_LINE`）、見出しは太字で字間を空ける（`secBar`）
    - 🔠 **文字の設定は `JS_FS_L` / `JS_FS_V` ＋ `JS_LW_W` / `JS_LH` ／
      `JS_VLH` / `JS_PAD_Y` / `JS_PAD_X` / `JS_VAL_MIN` の一かたまりだけ**
      （項目名 9px・太さ600・行間1.3 ／ 記入欄 9.5px・行間1.35 ／ 欄の余白 上下1px・左右6px ／
      いちばん低い高さ10px）。
      ⚠️ **項目名を太字にすると窮屈に見える**（実際に「余裕がなさそう」と言われた）ので、
      太字にはしない
    - 📐 **紙の下の余白を埋めるのは `_jsGrow` の1箇所だけ**（＝どの行にも同じだけ足す高さ）。
      **1回組んで高さを数える → 足りないぶんを1行ずつに配る → もう1回組み直す**の2回組み
      （`_jsBuildBlocks()`）。足す先は **`_jsPad()`＝欄の上下の余白**で、
      `_jsCellH()` も同じ `_jsGrow` を足すので**見積もりと実物が必ずそろう**
      - ⚠️ **いちばん低い高さ（`JS_ROW_BASE`）だけを上げても埋まらない。**
        項目名が2行に折り返している行には効かないため（実際に下が空いたままだった）
      - ⚠️ **埋めるのは「2列＝1枚におさまるとき」だけ**（`_h0 <= JOB_SHEET_COL_PX*2`）。
        2枚以上のときに埋めると、列に入りきらなかったぶんが押し出されて**紙が1枚増える**
        （記入の多い求人票で 2枚 → 3枚 になった）
      - 上限は `JS_GROW_MAX`(22)、はみ出し防止の余らせ幅は `JS_FILL_SAFE`(50)
    - ⚠️ **項目名と記入欄は1行の高さが違う**ので、`_jsCellH()` は**それぞれ別に数えて高いほう**を採る
    - ⚠️ **記入欄は「言葉の切れ目で折る」ので字数どおりには入らない**（`keep-all` / `auto-phrase`）。
      `_jsCellH()` は1行ぶん少なめに見ておく（見ないと紙からはみ出す。実際にはみ出した）
    - 📐 **紙の収まりを直すのは `JOB_SHEET_COL_PX`（995＝1列ぶん）の1箇所だけ**。
      ⚠️ これは**見積もりの単位**で、実物はこれより1割ほど低く出る（`_jsCellH()` を安全側に
      振ってあるため）。**実物の1列は A4縦(1123px) − 用紙の余白(10mm×2＝76px) − 見出し(約50px)
      − いちばん下の署名欄(約76px) ＝ 約920px** まで
      文字や幅の設定を変えたら、**空欄・5字・15字・25字・40字・60字**を流し込んで
      **空欄が1枚におさまること**と**どのページも A4（1123px）を超えないこと**を
      必ず測ってから決める（記入が増えれば2枚になるのは想定どおり）
    - ✂️ **文字の切れ目は `_jsWrap()` と `JS_WRAP_CSS` の2箇所だけ**＝**言葉の途中で改行しない**
      （「銀行決／済」ではなく「コンビニ決済・／銀行決済」）。区切り記号のうしろに見えない改行位置
      （U+200B）を入れ、CSS は `word-break:keep-all` →（新しいChromeでは）`auto-phrase`。
      どうしても1行に入らない語だけ最後の手段として途中で折る（`overflow-wrap:break-word`）
      - ⚠️ **◎●○△ の印の手前では折らない**（`⁠`＝word joiner ＋ `white-space:nowrap`）。
        入れないと**印だけが次の行に落ちる**（実際に落ちた）
    - 🏷️ **書類の題名はまん中に「求人票」だけ。ページ数（1 / 2）と種類（高度人材／一般）は右上の端っこ**
      （`head(n)` の1箇所。ページ数のほうを大きく、その下に種類）。ページ数は2枚以上のときだけ出す
    - ⚠️ **行の高さは px で見積もる**（`_jsCellH()`）。1行の高さは**その行でいちばん高い項目**で決まる
    - ⚠️ **折り返しの字数は列の幅から出す**（甘く見るとページからはみ出す。実際にはみ出した）
    - ⚠️ **記入欄には `line-height` を必ず書く**。書かないとブラウザの既定（日本語フォントで約2倍）
      になり、2行のつもりが3行ぶんの高さになる（実際になった）
    - 見出しだけがページの最後に残らないよう、**次の1行まで入るか**を見てから置く
    - 🎓 高度人材の**書類には英語の項目名を出さない**（既定）＝ 英語を出すと1枚に収まらないため。
      入力画面と参考用には出る。**出したいときは上の `_flyerShowEn()` のチェックを入れる**
      （`JOB_SHEET_DOC_EN` は常に出したいときの逃げ道）
...
- **種類を選ぶ場所は2つ。役割が違うので混ぜないこと**
  ...
  - 求人票作成で見ている種類は **`_flyerKindNow()` の1箇所**。求人を選び直すと `null` に戻る
    ＝その案件に保存されている種類に戻る
  - **求人を選んでいなくても種類は選べる**（空の様式を印刷して企業に渡せるように）。
    ⚠️ そのため `loadJobFlyer` の最後で、何も選ばなかったときに `renderFlyerByMode()` を呼んでいる
    （これが無いと `onFlyerJobSelect` が走らず、種類の行が出ない）
  - ［✏️ 項目を入力］（`flyerEditSheet`）は、**いま見ている種類のまま**求人案件の【📄 求人票】タブを開く
    （開いてから `_jpSheetKind` に入れる。保存は今までどおり 💾 のとき）

### 👥 求人案件の人数は2つある（募集人数 ／ 見込み人数）

- **名前と説明は `JP_COUNT_LABEL` / `JP_COUNT_NOTE` の1箇所だけ**

  | 列 | 画面の名前 | 意味 |
  |---|---|---|
  | `expected_count` | **募集人数** | 企業が何名ほしいか。**労働局指定項目**（`JP_LABOR_INFO_FIELDS` の ＊） |
  | `prospect_count` | **見込み人数** | 実際に応募があって提案できる人数（あとから増える） |

- ⚠️ **`expected_count` の列名は変えない。** 旧「見込（人数）＝ 求人数」の名前を変えただけで、
  **これまで入れてきた人数はそのまま残る**（求人票の「採用予定人数」・労働局 求人管理簿・
  求人票の外部フォームも、今までどおりこの列を見る）
- 一覧は **求人管理リスト**と**売上見込みの一覧**の2つに「募集」「見込」の列を出す。
  **求人管理リストのセルを作るのは `_jpCountCell()` の1箇所だけ**で、
  押すとその欄まで開く（`openJobProgressDetail(src_row, 'jpf_<列>')`。
  ⚠️ 行のクリックと二重に発火しないよう `event.stopPropagation()` が必須）
- 見出しと合計は「募集 ◯人 ・ 見込 ◯人 ・ 初期費用 ¥◯」の順で全部出す

#### 💰 初期費用（合計）＝ 初期費用単価 × 見込み人数

- **欄の名前は `JP_FEE_LABEL` の1箇所だけ**（`unit_price`＝初期費用単価（円）／`amount`＝初期費用（合計））。
  ⚠️ **列名は `unit_price` / `amount` のまま変えない**＝ これまで入れてきた金額がそのまま残る
  （売上見込みの一覧・手数料管理簿の算出根拠も今までどおりこの列を見る）
- **計算するのは `_jpFeeCalc()` の1箇所だけ**。単価か**見込み人数**（`prospect_count`）を直すと走る
  （例: 2,000円 × 2名 ＝ 4,000円）
  - ⚠️ **募集人数（`expected_count`）は金額に使わない**（以前は募集人数で計算していた）
  - ⚠️ **どちらかが空のときは合計に触らない**＝ 単価を入れていない過去の案件の金額を消さないため
  - 合計はそのまま手でも直せる（あとで単価か見込みを直すとまた計算し直す）
- **一覧に単価は出さない。** 直すときは「見込」か「初期費用」のセルを押す＝
...
### 📊 求人票のExcel出力（PDFと同じ票の形にする）

- ⚠️ **ExcelのHTML取り込みは `display:flex` / `display:grid` を読まない。**
  画面のHTMLをそのまま渡すと**すべての欄が縦1列に崩れる**（実際に崩れた）。
  Excelが分かるのは `<table>` だけなので、**出力の直前に flex / grid を
  `<table><tr><td>` に組み替える**（3種類の求人票も候補者向けチラシも同じ経路）
- **組み替えるのは `_flyerExcelHtml()` の1箇所だけ**（画面のHTMLには触らない。
  画面の外に同じ幅で置いたコピーを直す）。順番は
  ① `_xlMeasure()`（実寸と罫線を先に測る）→ ② 画像を落とす →
  ③ `_xlFlexToTable()` / `_xlGridToTable()`（flex・grid を表にする）→
  ④ **`_xlGridify()`（入れ子をやめて「1枚の表」にたたむ）**
- ⚠️ **表を入れ子にしたまま渡してはいけない。** Excelは内側の表を**上下に積むだけ**なので列がそろわず、
  **A列とB列に項目名と記入欄が縦に並ぶ**（実際になった）。
  **`_xlGridify()` が画面での位置（px）から縦横の罫線を拾い、`colspan`／`rowspan` 付きの
  1枚の表に組み直す**（`<colgroup>` に列幅を入れるのでExcelの列幅もそろう）
  - **セルを拾うのは `_xlLeafCells()` の1箇所**（表をこれ以上含まない要素まで下りる）
  - **罫線の位置を決めるのは `_xlBounds()` の1箇所**（`XL_SNAP`(3)px 以内のズレは同じ線とみなす）
- ⚠️ **作り替える前に実寸をまとめて測ること**（1つ直すたびに配置が変わって測れなくなる）。
  flex の比率はExcelに伝わらないので、**幅は px で入れる**
- ⚠️ **入れ子の表にしない。** 子がすでに1行の表になっているとき（「項目名｜記入欄」）は、
  **そのセルをそのまま横にならべる**（`_xlAppendKid`）。入れ子にするとExcelがうまく開けない。
  そのとき**外側が持っていた下・右の罫線は、ならべ直したセルに配り直す**
  （やらないと区切り線と外枠が消える。実際に消えた）
- ⚠️ **画像は1つ残らず落とす。** Excelは外部URLの画像を出せず、
  「リンクされたイメージを表示できません」が出る（実際に出た）。透かしのロゴも同じ
- Excelに渡さないCSS（レイアウト系）は **`XL_DROP_CSS` の1箇所**
...
- 📄 **出す質問は「その案件の求人票の種類」で変わる。決めるのは `_jobFormKind()` の1箇所だけ**
  （中身は `jobSheetKindOf(job_progress)`。社内の求人票タブと必ず同じ種類になる）

  | 種類 | 出す項目 | 欄のid | 保存先 |
  |---|---|---|---|
  | 🌏 特定技能 | `JOB_SHEET_GROUPS` | `jsf_<列名>` | `job_progress` の列そのもの |
  | 🎓 高度人材 ／ 📋 一般 | `jobSheetGroups(kind)` | `jsx_<キー>` | `job_progress.sheet_ext[kind]`（`_jsExtSet()` を通す） |

  - ⚠️ **種類を見ずに `JOB_SHEET_GROUPS` を出すと、高度人材・一般の案件でも
    特定技能の質問が並ぶ**（実際に並んだ）
  - 1項目を作るのは **`_jobFormExtFieldHtml()` の1箇所**。項目の定義は社内と同じものを使うが、
    **社内向けの印（◎労働局指定・△紹介事業者）と未記入の薄赤は出さない**（記入する方には関係がないため）
  - 送信は**書かれた項目だけを上から重ねる**（`sheet_ext` を丸ごと入れ替えない）
  - 種類はフォームの見出し（帯の下）と、社内の「🔗 外部記入リンク」の欄の両方に印で出す。
    欄の印は **`_jpSheetKindNow()`＝いま画面で選んでいる種類**（上のボタンと必ず同じ）
  - ⚠️ **リンクを発行するときは、選んだ種類を先に `sheet_kind` に書く**（`createJobSheetLink` の1箇所）。
    種類の選択は 💾 保存までメモリにしか無いので、これが無いと
    **高度人材を選んでいるのに特定技能の質問のリンクが出る**（実際に出た）。
    発行の確認にも「出る質問は【🎓 高度人材】の項目です」と出す
...
- 外部フォームの入力欄idは社内タブと同じ（🌏 特定技能＝`jsf_*` ／ 🎓📋＝`jsx_*`。
  写真の部品も共用しているので**ずらさないこと**）
...
- 見出しは「求人票」。左上は **`_flyerLogoHead(kind, 分野)` の1箇所**＝ KMTロゴ ＋ 名前 ＋ 職業紹介番号
  - **名前は求人票の種類で変える。対応表は `FLYER_HEAD` の1箇所だけ**
    （🌏 特定技能＝`登録支援機関KMT` ／ 📋 一般・🎓 高度人材＝`株式会社KMT`）
  - **番号は `KMT_AGENCY_LICENSE_NO` の1箇所だけ**（3種類とも共通）。
    一般・高度人材の「有料職業紹介事業 許可番号」の欄にも `JOB_SHEET_EXT_FILL` から自動で入る
  - 🏢 **この分野だけ「一般社団法人KMT」＋無料職業紹介番号にする。
    対象と番号は `JOB_SHADAN_FIELDS`（建設・自動車運送）／`KMT_SHADAN_LICENSE_NO`(13-ム-300152) の1箇所だけ**、
    **判定は `_isShadanField(分野)` の1箇所だけ**（左上の名乗り・番号・ロゴ・背景の透かしが同時に追従する）。
    分野は**前方一致**で見る（「建設」「建設分野」「自動車運送業」のどれでも拾う）
    - **案件から分野を取り出すのは `_jobFieldOf(j)` の1箇所**（求人案件・求人登録とも `industry`）。
      ⚠️ `_flyerLogoHead()` / `_flyerWatermark()` を**新しく呼ぶときは必ず分野を渡すこと**
      （渡さないと株式会社KMT扱いのまま出る）
...
### 📣 募集リンク（1本を全員で共有する応募フォーム）

- **これまでと順番が逆**。候補者を先に作らず、**本人が送信した瞬間に候補者が1件できて、
  その人専用の続きリンクも自動で出る**
  | | 入口 | 順番 |
  |---|---|---|
  | これまで（今までどおり） | 候補者詳細【📝 履歴書作成】 | 候補者を作る → リンク発行 → 本人が記入 |
  | 📣 募集リンク | 候補者管理【📣 募集リンク】タブ | リンクを配る → 本人が記入・送信 → **候補者ができる** |
- ⚠️ **これまでの1人1本のリンク（`kind='resume'` / `'info'`）には1文字も触っていない。**
  見分けるのは **`kind` の1つだけ**で、募集リンクは **`candidate_id` が null**
  （そのために `candidate_form_links.candidate_id` の NOT NULL を外した）
- **ページは0/1/2の3つ。定義は `CAND_APPLY_PAGES` の1箇所だけ**
  ＝ 0 必須（**これだけで応募できる**）／1 履歴書①／2 履歴書②（人材資料）
  - **0ページ目の項目は `APPLY_P0` の1箇所だけ**（欄のidは `af_<列名>`）。
    ⚠️ **受付年月日・有効期間は出さない**（社内が入れる欄なので、応募者には書けない）
  - 1・2ページ目は**社内の履歴書とまったく同じ部品**を使う＝項目の定義を二重に持たない。
    `resumeFormFieldsHtml({external:true, docs:false})` ＋ `resumeDocBoxHtml()`
    - **`docs:false` は募集フォームのためだけの逃げ道**。既定は今までどおり人材資料も出す
  - **どこまで書いてもらうかはリンクの `max_page`**（発行するときに決める）
  - 1ページ目に入るとき `fillResumeForm(_applyVals)` で**0ページ目の内容を写す**
    （`CAND_SYNC_FIELDS` と同じ考え方。氏名・国籍などを2回書かせない）
- 🗓 **受付日＝送信した日／有効期限＝そこから `CAND_APPLY_VALID_MONTHS`(6) か月後**。
  **計算は `candValidUntil()` の1箇所だけ**（月末をまたぐときは末日に寄せる）
- 🚫 **二重登録のくらべ方は `_candDupKey()` の1箇所だけ**（空白と大文字小文字を無視。
  人材の `_dupNameKey()` と同じ考え方）。探すのは **`findCandDup()` の1箇所**で、
  **手元のリストではなくDBに聞く**
  - 有効期限**内**の同じ人がいる → **登録させない**（赤で知らせる）
  - 期限**切れ**の同じ人 → 黄色で知らせて**そのまま登録できる**
  - ⚠️ **送信の直前にもう一度見る**（書いている間に別の人が登録しているかもしれない）
- 👥 **応募が何人来たかは数え直して出す**（`_applyCounts`）。リンクの行にカウンタを持たせると
  同じ時間に2人送信したときに数がずれるため。**自動でできた続きリンクの `label` に
  募集リンクの `token`** を入れてあるので、それを数えればよい（`APPLY_AUTO_BY` で絞る）
- 🧪 **テスト用リンクは `is_test=true` の1本だけ**（`APPLY_TEST_LABEL`）。
  **これを社員みんなに配って試してもらう**＝募集リンクはもともと1本を全員で使うものなので、
  試すときも同じ形にする。
  ⚠️ **人ごとに1本ずつ作らない**（以前GLT13名ぶんを作ったが、配る手間が増えるだけで
  試したいこと自体は変わらないのでやめた）
  - ここから登録された候補者は**氏名の頭に `TEST_CAND_PREFIX`（【テスト】）が付く**
...
- 🌏 **国籍の選択肢は `CAND_NATIONALITIES` の1箇所だけ**（＋ `CAND_NAT_OTHER`）。
  ...
  ⚠️ 一覧に無い値が入っている候補者もいるので、`candNationalityOptionsHtml()` は
  **いまの値が一覧に無ければ先頭に残す**
...
### 🔢 求人案件の案件番号（`job_progress.src_row`）

- `src_row` は**求人案件の業務キー**。候補者リスト（`candidate_job_progress.job_src_row`）・
  求人票の外部記入リンク・求人チャット・求人ノートが**全部これで結び付いている**ので、
  **同じ番号の行が2つあってはいけない**
- ⚠️ **採番するのはDB**（`job_progress_src_row_seq` を `src_row` の DEFAULT にしてある）。
  **クライアントからは `src_row` を送らないこと。**
  DBには **`job_progress_src_row_uniq`（一意）** も付けてあるので、ぶつかれば保存が落ちて気づける
  - > ⚠️ **過去の事故**: 以前は登録するときに
    > 「**手元の一覧（`allJobProg`）の最大値＋1**」で番号を決めていた。そのため
    > **①同じ時間に別の人が登録した ②［登録］を2回押した ③一覧が古いまま**のどれかで
    > **同じ番号の案件が2件**でき、削除は `src_row=eq.◯` の PATCH だったので
    > **片方を消すと両方消えた**（2026/09/02〜03 に実際に起きた。7組の重複ができていた）
  - 番号を決めていたのは**3か所**（新規案件登録・売上見込みからの新規登録・
    求人ヒアリングフォーム）。**足すときも絶対に自分で採番しない**
  - POST の返り（`return=representation`）に `src_row` が無ければ**そこで止める**
    （番号の無い行は開くことも消すこともできない）
- ⚠️ **消すときは必ず `id`（主キー）で1件だけ**（`deleteJobProgress`）。
  `src_row` で消すと、万一重複したときに巻き添えになる
- ⚠️ **二重クリックで2件作らせない**＝ `_jpSavingNew` の1箇所で止め、ボタンも無効にする
  （`finally` で必ず戻す。戻さないと失敗したあと二度と登録できない）
...
- 🏢 **ロゴの種類は `KMT_LOGOS` の1箇所だけ**（`corp`＝株式会社KMT ／ `shadan`＝一般社団法人KMT）。
  **画像を読むのは `_kmtLogoSrc(kind)` の1箇所だけ**で、履歴書も求人票も透かしもここを通る
  - `corp` … ヘッダーの `<img class="logo-mark">` を流用する（画像を二重に持たない）
  - `shadan` … **`app_settings.kmt_logo_shadan`**（data URI）。読むのは `loadKmtLogos()` の1箇所
    （起動時に1回）。⚠️ **kmt.html に2枚目の base64 を埋めない**（さらに重くなるため。
    電子印 `app_settings.reg_org_seals` と同じ考え方）
  - **登録は管理者だけ**＝ `openKmtLogoModal()`（【📝 履歴書作成】の「🏢 履歴書の見た目」の
    ［🏢 ロゴを登録］）。上限は `KMT_LOGO_MAX_KB`(300) の1箇所
  - ⚠️ **登録するまでは `corp` のロゴで出す**（`_kmtLogoSrc` の中）＝黙って画像が消えないように。
    登録されているかを見るのは `_kmtLogoReady(kind)` の1箇所
- **履歴書がどちらのロゴを使うかは `_resumeLogoKind(c)` の1箇所だけ**。保存先は
...
- ⚠️ **予定の取得は `_fetchAllSchedules()` の1箇所。1000件ずつ最後まで読む。**
  Supabase（PostgREST）は1回のGETで**既定1000件までしか返さない**ので、そのまま取ると
  `scheduled_at` の**古い順に1000件で打ち切られ、新しい月が丸ごと出なくなる**
  （実際に「2026年9月 0件 / 全1000件」になり、予定が消えたように見えた）。
  **ほかの一覧でも件数が1000を超えるときは同じやり方にすること**
- 📅 ［＋ 登録してカレンダーに追加］は **先にシステムへ保存してから** Googleカレンダーを開く
  （`_schedOpenGcal()` の1箇所）。**カレンダー側で登録しなくても保存は残る**。
  ⚠️ `noopener` を付けた `window.open` は開けても必ず null を返すので、
  **開けたかどうかは判定しない**（判定すると「止められました」と嘘を出す。実際に出た）。
  代わりに `_schedGcalLink()` が［📅 Googleカレンダーを開く］のボタンを必ず出す
- 👥 **新規登録のときだけ人材を複数えらべる**（定期面談などを1社まとめて入れるため）。
  ⚠️ **データの形は変えていない**＝ 保存すると**人数ぶんの予定を1件ずつ**作るので、
  1人ずつ登録したのとまったく同じものができる（1予定＝1人材＝1記録）。
  面談記録・定期面談報告書（5-5号/5-6号）・相談記録書（5-4号）は**影響を受けない**
  - **選んでいる人材は `_schedPickedWorkerIds` の1箇所だけ**に覚える
    （書き替えるのは **`_schedSetPickedWorkers()` の1箇所**。`_schedPickedWorkerId` は先頭の1名で、
    企業の自動セットに使う）。⚠️ 検索や企業で `<select>` を描き直すと選択が消えるので、
    `renderSchedWorkerOptions()` が覚えてあるぶんを付け直す
  - ⚠️ **選択のたびに `<select>` を描き直さない**（Shift の範囲選びが切れる）。
    人数の表示だけ **`_schedWorkerHint()` の1箇所**で更新する
  - **複数えらべるかは `_schedMulti` の1箇所**（新規＝true／編集＝false）。
    ［☑ この企業の人材をすべて選ぶ］は `schedPickAllWorkers()`（**すでに選んだ人は残して足す**）
  - **人数ぶん作るのは `_schedSaveMany()` の1箇所だけ**。1件ずつ
    ①`support_schedules` に POST ②`_saveMeetingFromSchedule()` ③`_syncScheduleToWorker()`
- 🏢 **企業名で絞り込んで1社だけになったら、その企業をそのまま選ぶ**
  （`renderSchedCompanyOptions()` の中の1箇所）＝ 人材もその企業のぶんだけになる。
  ⚠️ **一覧に出ているだけでは `<select>` は選ばれない**ので、これが無いと
  **企業名を打ったのに人材が全員のまま**になる（実際になった）。
  すでに選んでいる企業が候補から外れたときも選び直す
  - いま選んでいる企業は企業の欄の下（`schedCompanyHint`）に出す。
    ［解除］は **`schedClearCompany()` の1箇所**で、⚠️ **検索の文字も一緒に消すこと**
    （残っていると1社に絞られたままなので、上の自動選択がすぐまた選び直して解除できない）
    を通す＝**1名のときの経路とまったく同じ関数**を人数分呼ぶだけ
    - ⚠️ **同じ人材・同じ種別・同じ日の予定がすでにあれば飛ばす**（先に1回のGETでまとめて調べる）。
      飛ばした人の名前もトーストに出す（黙って落とさない）
    - ⚠️ **Googleカレンダーは人数ぶん開かない**（タブがブラウザに止められる）＝
      **全員の名前を入れた1件ぶんのリンク**を出すだけ（`_schedGcalLink(url, 人数)`）。
      訪問は1回なので、カレンダー側が1件のほうが実態にも合う
    - モーダルは閉じない（ほかの保存と同じ）。もう一度押されても重複チェックで二重にならない
  - 🗓 **カレンダーの見え方は変えていない**＝ 1名＝1予定なので人数ぶん並ぶ。
    企業でまとめて「◯◯社 3名」の1行にするなら `renderScheduleMonthGrid` にまとめる処理が要る（未実装）
- 📅 **一覧に出す日付の1行は `schedDateText(s)` の1箇所だけ**
  （人材の【支援スケジュール】【支援履歴】・所属機関の【支援スケジュール】が必ず同じ見た目になる）
  - ⚠️ **見出しは「予定:」ではなく「日付:」**。予定モーダルは予定日と実施日を1つの「日付」に
    まとめてあるので、**実施済みの行まで「予定:」と出て分かりにくかった**（実際に言われた）
  - ⚠️ **実施日が予定日と同じときは「／実施:」を出さない**
    （`saveSchedule` は実施済にすると日付をそのまま `completed_at` にも入れるので、
    書くと同じ日付が2回並ぶ）
- 🔗 **予定（`support_schedules`）は1つのデータ。出る場所は4つ**＝ 📅 支援スケジュールの画面／
  外国人材【支援スケジュール】【支援履歴】／所属機関【支援スケジュール】。
  **どこからでも 追加・編集・削除ができ、どれも同じ1行を見る**（別テーブルを作らない）
  - **直したあとの反映は `_schedAfterChange(workerId, companyId)` の1箇所だけ**＝
    開いている一覧（支援スケジュール／人材の予定／支援履歴／所属機関の予定／面談記録）を全部取り直す。
    保存（`saveSchedule`）・実施済（`markScheduleDone`）・削除（`deleteSchedule`）が必ずここを通る
  - **1件を引くのは `_schedRowById(id)` の1箇所**（手元に無ければDBから引く）。
    ⚠️ `_supportSchedules` は支援スケジュールの画面を開いたときにしか埋まらないので、
    **`_supportSchedules.find()` だけで済ませない**（人材・所属機関のタブから
    ［✓ 実施済］を押しても黙って何も起きなかった。実際に起きた）
- 🗑 **予定の削除は `deleteSchedule(id)` の1箇所だけ**（一覧の🗑と、予定モーダルの［🗑 削除］が同じものを通る）
  - 消す前に**1回だけ**たずねる（支援履歴の `deleteWorkerHistory` は2回。あちらは人材の画面から
    カレンダーごと消すため）。文面を作るのは **`_schedDeleteConfirmText()` の1箇所**で、
    種別・予定日・人材名・企業名を出す＝**どれを消すのか確かめてから押せる**。
    メモリに無い予定は1件だけ引いてから聞く（黙って「この予定」とだけ出さない）
  - 予定モーダルの［🗑 削除］は**編集のときだけ**出す（新規は消すものが無い）。位置はフッター統一ルールどおり
    `📅 カレンダーに追加 │ 余白 │ キャンセル・💾 更新 │ 余白 │ 🗑 削除`
  - 消したら `forceCloseModal('scheduleModal')` で閉じ（未保存の確認は出さない）、
    反映は `_schedAfterChange()` に任せる（開いている一覧を全部取り直す）
  - ⚠️ **人材情報【8. 支援情報】の日付と、面談記録（`company_meetings`）の中身は消えない**。
    別のものなので、確認の文にもそう書いてある
- **表示は 週／月／年 の3つ**（並びは ［📍 今日］→ 週 → 月 → 年）。ボタンとの対応は **`SCHED_CAL_VIEWS` の1箇所**、
  期間の出し方は **`_schedCalRange()` の1箇所**、［◀ 前］［次 ▶］のずらし幅は
  **`_schedCalShift()` の1箇所**（年＝1年／週＝7日／月＝1か月）
  - 年表示は `renderScheduleYearGrid()`＝12か月ぶんの小さなカレンダー。
    予定は**種別の色の点**（4件以上は「+n」）、月の見出しを押すと `schedCalGoMonth()` で月表示に切り替わる
  - 件数の見出しの言い方は **`SCHED_PERIOD_LABEL` の1箇所**（この年／この週／この月）
- 📅 **社内の「年間カレンダー」の休みを、支援カレンダーにも出す。**
  **読むのは `loadCalDays()` の1箇所だけ**で、`company_calendars` の
  `days`（日付ごとの区分）と `holidays`（◇祝日）の**両方**を見る。
  その日が休みかは **`calDayOff(ymd)` の1箇所**、色は **`_calOffStyle()`**＝
  `CAL_DAY_KINDS` をそのまま使うので**年間カレンダーと必ず同じ色**になる
  - 年・月・週のどの表示にも出る。凡例は **`_schedCalOffNote()` の1箇所**
    （その年度が未登録のときは「まだ登録されていません」と出す）
  - 読み込みは `loadSupportSchedules()` の中で1回（`_calDayMap` に持つ）
- カレンダーの上の凡例は `renderScheduleLegend()`。**表示中の期間（年／週／月）の件数を種別ごとに出す**。
...
- 中身は **`support_schedules` の同じ行**（別テーブルを作らない）。
  `loadWorkerHistory` → `renderWorkerHistory` で、並べ替え・絞り込みだけを付けたもの
- 行ごとに ✏️（`openScheduleModal`）と **🗑（`deleteWorkerHistory()` の1箇所）**
  - ⚠️ **論理削除ではなく行ごと消える**（`support_schedules` を DELETE する）。
    **消す前に必ず2回たずねる**（`_confirmDeleteTwice()`）
  - 確認の文には「📅 支援スケジュールからも消えること」と、
    **人材情報【8. 支援情報】の日付と面談記録（`company_meetings`）は消えないこと**を書く
    （別のものなので、必要ならそれぞれの画面から消してもらう）
  - 消したあとは、開いている支援スケジュールのカレンダーと人材の予定一覧も取り直す

#### ✍️ 確認書の電子サインのリマインド（事前ガイダンス／生活オリエンテーション）

- 予定の種別に **事前ガイダンス**・**生活オリエンテーション**をえらぶと、種別のすぐ下に
  「✍️ 電子サインもお願いします！」の枠が出て、**えらんだ人材が1行ずつ**並ぶ
- **種別 → 確認書の様式の対応は `SCHED_SIGN_TASKS` の1箇所だけ**
  （事前ガイダンス → 5-9号＝記録簿 no.4 ／ 生活オリエンテーション → 5-8号＝記録簿 no.7）。
  ⚠️ **`sign:true` の様式を足したらここにも足すこと**（`REG_SIGN_ITEMS` と対になる）
- **署名の状態は `_schedSignStateOf()` の1箇所**（`none` 未発行／`wait` ⏳ 署名待ち／`done` ✍️ 署名済み）。
  見た目は **`SCHED_SIGN_STATES` の1箇所**
  - ⚠️ **署名ずみかどうかは `signed_at` で見る**（`signature` は画像なので引くと重い）
  - ⚠️ 無効にしたリンクは**「未発行」と同じ扱い**（出し直せるようにするため）
  - 引くのは **`_loadSchedSignState(no, ids)` の1箇所**＝ **1回のGETで全員ぶん**。
    同じ人に何本かあれば**いちばん新しい1件**だけを見る
- 枠を出すのは **`renderSchedSignRemind()` の1箇所**（種別を変えたとき・人材を選び直したとき・
  保存したあとに呼ぶ）。枠の色は**その種別の色**（`SCHEDULE_EVENT_TYPES`）にそろえる
- ［✍️ ◯号をひらく］は **`openSchedSignDoc(no, workerId)` の1箇所**＝
  記録簿の画面へ移って、**その人材の記録があればそれを、無ければ人材を入れた新規**の記入フォームを開く
  - ⚠️ **署名リンクは記録を保存してからでないと発行できない**（`createRegSignLink` が `record_id` を要る）。
    新規で開いたときはトーストでその旨を知らせる
  - ⚠️ 予定の入力が未保存のときは確認してから移る（`_manualSave['scheduleModal'].dirty`）
- 🔔 **予定日が過ぎてもまだ署名がとどいていない人材**のお知らせ＝
  `loadSignTodos()` → `updateSignAlerts()`（サイドバー `signAlerts` ／ マイページ `mySignAlerts`）。
  一覧は `openSignTodoList()`
  - **同じ人材・同じ様式は、いちばん新しい予定1件だけ**にする（何度も面談していても1行）
  - 出す相手は**自分の担当ぶんだけ**＝ 担当の判定は面談記録の記入待ちと同じ
    **`_meetingTodoIsMine()` を使い回す**（判定を2つ持たない）
  - 担当マークは **GLT**（`ALERT_OWNER_OF` の `signTodo`）
  - 読み込むのは面談記録の記入待ちと同じ3か所（起動時・マイページ・記録を直したとき）
  - 🔕 **いまは出していない。出す・出さないは `SIGN_TODO_ALERT_ON` の1箇所だけ**（`false`）。
    電子サインはこれから始めるので、**過去の予定がまとめて残って160名を超えていた**ため。
    `loadSignTodos()` と `_signTodoMine()` の先頭で見るので、`true` に戻すだけで元どおり出る。
    ⚠️ **予定モーダルのリマインドと【✍️ 署名リンク】タブは止めていない**（お知らせだけ）
...
- `decide:true` … 更新決定チェックの［決定］ボタンに出すもの（5つ）。
  申請依頼済み・完了は決定の対象ではないので付けていない
  - 🎓 **更新決定（6年目）** … 5年満了のあと、通算6年目の更新をする人。
    `REN_APPROVAL_MATRIX` と `REN_APPLY_LATE_FROM` にも入れてあるので、
    決定ずみからの変更は承認が要り、⏰ 申請依頼まちにも出る
...
- **確認事項（質問・選択肢・出す条件・必須かどうか）は `REN_CHECK_ITEMS` の1箇所だけ。**
  ...
  - ⚠️ **開くときは在留期限が入っている人材の行を必ず使う**（`openRenewalCheck` の先頭で、
    `allWorkers` の行に `visa_expiry` が無ければ取り直す）。
    画面によっては `allWorkers` が軽いSELECTで上書きされていて、そのまま対象月を出すと
    **空になり、別のシートとして保存されてしまう**（`target_ym=''` のシートが4件できていた）
  - 📋 **前回のシートを引き継ぐ。** その対象月のシートが無ければ、
    **その人材のいちばん新しいシート1件**（前の回ぶん／対象月が空のまま保存されたぶん）を
    そのまま流し込む（`_renCheck.carried`）。
    - **引き継いだときは `id` を持たない＝保存すると今回のぶんとして新しく作る**
      （前回のシートは書き替えない）
    - **引き継いだことは必ず画面に出す**（どの回のだれの記入かを緑の帯で出す）。
      黙って前の内容が入っていると、今回確認したものと見分けがつかない
  - **退職予定日・一時帰国日・5年満了は人材情報にも書き戻す**
    （対応は **`REN_CHECK_TO_WORKER` の1箇所**＝
    `resign_expected_date`→`expected_leave_date` / `temp_return_date`→`return_home_date` /
    `temp_return_back_date`→`return_jp_date` / `y5_expiry`→`tg2_expiry` / `y5_status`→`tg2_status`）。
    欄とチェックシートが必ず同じものを指すようにするため。
    **開いたときは空の欄だけ人材情報から持ってくる**（`openRenewalCheck`）
  ...
  - **①ステータスを先に選び → ②確認事項を記入 → ③［💾 保存］で反映**の順。
    ステータスのボタンは**記入がすんでいなくても押せる**（先に選ぶ順番なので）。
    記入もれがあると保存しても反映しない（`decideRenewalFromCheck` が止める）
  - ⚠️ **ステータスを選ばないまま［💾 保存］したときに、黙って終わらせないこと。**
    以前は `saveRenewalCheckAll` が `if (!change) { … return; }` で**何も知らせずに返していた**ので、
    確認事項だけが `renewal_checks` に入り、`workers.renewal_status` は空のまま＝
    **決めたつもりなのに更新リストから消えない**（実際に FATAH さん・FIRMAN さんで起きた。
    `renewal_status_log` も `renewal_status_requests` も0件だったので、押されていないと分かった）。
    いまは**まだ一度も決まっていない人材**（`renewal_status` が空）のとき、
    ① チェックシートに赤い注意書きを出す ② 保存ボタンを［💾 保存（ステータス未選択）］にする
    ③ 保存の前に `askChoice` でたずねる（［① ステータスを選ぶ（戻る）］／［確認事項だけ保存する］）
    ④ 確認事項だけ保存したときはトーストで知らせる、の4つで気づけるようにしてある
  - 🔁 **選んだステータスで、聞く確認事項が変わる。分けるのは `REN_CHECK_ONLY` ＋
    `_renCheckItemsFor(status)` の1箇所だけ**＝【更新なし】のときは
    退職・5年満了まわり（`g:'resign'`）だけを出す。
    **未入力の数（`_renCheckMissing(vals, status)`）も同じものを見る**ので必ず一致する
  - **質問の番号は、いま出ている欄に上から通し**で振る（`renderRenewalCheck` の1箇所）。
    項目に `no` を持たせない＝欄の出し入れと必ず合う
  - 🎓 **本帰国予定・転職予定を選んだら、更新のための確認事項は聞かない**
    （一時帰国・税書類・収入印紙費用）。**`REN_Y5_LEAVE` ＋ `_renY5Leaving(v)` の1箇所だけ**で、
    欄が消えるので未入力にも数えない＝そのまま保存できる
  - 🎓 **2号変更予定のときだけ聞くことは `REN_Y5_2GO*` ＋ `_renY5Is2go(v)` の一かたまりだけ**＝
    2号に変わったあとの支援（引き続きKMT支援／KMT支援終了）→ 引き続きのときだけ委託費用
    （現在と同じ／変更あり）→ 変更ありのときだけ変更の内容（記入欄）
  - 📝 **そのほかのメモは `g:'always'` なのでどのステータスでも出る**
    （更新なしのときも書けるようにするため。`_renCheckItemsFor()` が必ず残す）
  - 🎓 **5年満了は `REN_Y5_PLANS` ＋ `_renIs5y(v)` の2箇所だけ**。
    ［☑ 5年満了］（`type:'check'` ＝ `renRcToggle()`）を押すと、
    5年満了日／5年満了のあとの予定／5年満了状況 が出る。
    ⚠️ **1つめの質問の選択肢に【5年満了】は置かない**（チェックとダブるため）。
    以前そうしていた時期のデータのために `_renIs5y()` は `resign_plan` も見る。
...
- 🚫 **更新決定の対象にしない在職ステータスは `REN_EXCLUDE_STATUS` ＋
  ...
  更新決定リスト（`_renCycleTargets`）と ⏰ 申請依頼まち（`_renApplyLate`）が同じものを通る
  （退職した人材が更新決定リストに出ていた）
- 🚫 **更新決定の対象にしない在留資格は `REN_EXCLUDE_VISA` ＋ `_renExcludedVisa(w)` の1箇所だけ**
  （いまは `特定活動`。**前方一致**で見るので「特定活動（就職活動）」のような書き方も拾う）＝
  特定技能1号の更新ではないので、毎月の更新決定にはかけない
  - ⚠️ **見るのは `_renCycleTargets` だけ。⏰ 申請依頼まち（`_renApplyLate`）には掛けない**＝
    すでに【更新決定】が入っている人材は、特定活動でも申請の依頼もれとして出し続ける
  - 繰り越し（`_renIsCarry`）は `REN_CARRY_VISA`＝特定技能1号 しか通さないので、もともと入らない
  - 対象外にしたものは**一覧の上の案内にも出す**（`renderRenewalCycle` の回ごとの見出しの
    ※の1行。文言は `REN_EXCLUDE_VISA` から作る）＝「いるはずの人が出ない」を黙って起こさないため
...
### 💰 初期費用と変更履歴（所属機関の【契約・委託費】のいちばん上）

- 場所は **所属機関の詳細【契約・委託費】タブのいちばん上**（💴 委託費設定の上）。
  見出しの通し番号は `numberCoSections()` が振るので、**以降の番号は自動で繰り下がる**
- 実体は **`companies.initial_fee`（いまの金額）＋ `initial_fee_log`**（jsonb の配列。別テーブルを作らない
  ＝企業の保存と一緒に入る）。1行＝1回の変更で `{date, from, to, staff, note, at, by}`
- **金額を書き替えると履歴に1行できるのは `onCoInitialFeeChange()` の1箇所だけ**
  （欄の `onchange`）。今日の日付・変更前・変更後・自分の名前が入った行を**先頭に**足す
  - くらべる相手は **`_coInitFeeBefore`（開いたときの金額）の1箇所だけ**（`fillCoForm` で入れる）
  - ⚠️ **同じ変更で2行にしない**＝ 先頭の行が「変更前が同じ・日付が今日」なら**変更後だけ書き替える**
    （100,000 → 120,000 → 130,000 と続けて直しても1行のまま）
  - **書き込むのは 💾 保存のとき**（その場では画面に足すだけ）
- 読み書きは **`renderCoInitFeeLog()` / `getCoInitFeeLog()` の2つだけ**。並びは**変更日の新しい順**、
  **中身が何も入っていない行は保存時に落とす**。［＋ 変更を追加］（`addCoInitFeeRow()`）で
  **過去のぶんも手で入れられる**
- 対応担当は `<input list="coStaffDl">` なので名簿の候補から選べて手入力もできる
  （候補は引継ぎの記録と同じ `_coStaffDatalistHtml()`）
- 【変更履歴】タブにも出す＝ `CO_WATCH_FIELDS` に `initial_fee: '初期費用'` を入れてある
...
#### 💬 チャットに送る（所属機関の【💴 入管申請費用】の下）

- ［💬 チャットに送る］で、**画面に出ている負担パターンとメモをそのまま**その企業のチャットに投稿する
  （`sendVisaFeeToChat()`）。送る前に文面とメンション先を出して1回たずねる
- **文面は `_visaFeeChatText()` の1箇所だけ**（負担パターンの文字は `visaFeePatternText()` を通すので
  画面と必ず同じ言い方になる）
- **メンションする相手は `_visaFeeChatMentions()` の1箇所だけ**＝ その企業の**メイン担当・サブ担当**。
  サブ担当は「A, B」のように並んでいることがあるので区切って読む（重複は落とす）。
  `mentions` に名前をそのまま入れる（通知の判定は `isMentionForMe()` ＝ `mentions` を見る）
- **企業チャットの部屋を用意するのは `_ensureCompanyRoomId()` の1箇所だけ**
  （企業チャット＝ `company_id` が入っていて `worker_id` が空の部屋。人材の `_ensureWorkerRoomId()` と同じ作り）
- 送ったあとは欄の下に「✅ チャットに送りました ／ 💬 チャットを開く」を出す（画面だけ・DBに列は持たせていない）。
  ⚠️ 開くときは **`_pendingChatRoomId` に入れてから `showScreen('chat')`**
  （`showScreen` → `openChatRoom` を続けて呼ぶと初期化に上書きされる）
- ⚠️ **所属機関を保存する前は使えない**（`editingCompanyId` が無いときはトーストで知らせる）
...
- **毎月のサイクル**。日付を出すのは **`_renCycleDates(ym)` の1箇所**
  ...
  - 一覧の**企業の欄は `_renCoCellHtml(w)` の1箇所**＝ 企業番号 ＋ 企業名。
    ⚠️ **`w.companies` の embed に頼らないこと**（軽いSELECTで読み込んだ人材には付いていないので
    「-」になる。実際になった）。`coOfRow()` で引き直す
  - **氏名・企業名を押すとその人材／所属機関が開く**（`renCycleOpenWorker` / `renCycleOpenCompany` の2つだけ）。
    ⚠️ 更新決定のモーダルは人材詳細より上に出るので、**先に閉じてから開く**
  - ⚠️ **名前くらべは `_renSameName()` の1箇所だけ**（担当欄が「A, B」のときは `_renHasName()`）。
    **名簿は「蛭田　裕紀」なのに所属機関の担当欄は「蛭田」**のように名字だけ／フルネームで
    食い違うので、空白を無視したうえで**前方一致でも同じ人とみなす**。
    これが無いとその人の担当が1件も当たらず、**その人にだけお知らせが出ない**
    （蛭田・白井・高村・亀谷・坂齊で実際に出なかった）。
    使うのは 🔁 更新決定・⏰ 申請依頼まわりだけ（`_renNameIn` も同じものを見る）
...
- 一覧の上の検索欄（`renCycleQ`）。**探す文字を作るのは `_renCycleRowText(r)` の1箇所だけ**
  （回・管理番号・氏名・企業番号・企業名・在留資格・在留期限・更新ステータス・申請ステータス・
  メイン／サブ担当・お願いした相手・決定ずみ／未対応・繰り越し・テスト）。
  **列を足したらここに足せば、検索にもそのまま効く**
- 空白（半角・全角）で区切ると**すべてを含むもの**にしぼる（`_renCycleFilter()`）
- ⚠️ **しぼるのは一覧だけ。** 見出しの「残り ◯名」・担当ごとの内訳・サイドバーのお知らせは
  今までどおり全体で数える（検索で件数が変わって見えると、決まったかどうかが分からなくなる）。
  そのため検索中はその旨を欄の横に出す
- 打つたびに描き直すので、**フォーカスとカーソル位置を戻してから返す**（`setRenCycleQ()`）。
  戻さないと1文字ごとにフォーカスが外れて打てない
- 0件のときは「✅ この回はすべて決定済みです」と出さない（検索で消えただけなので）
...
- 更新の管理者（`REN_CYCLE_ADMINS` ＝ ニサ・白井・アヒュ）は、**自分の担当ぶんとは別に**
  「依頼をかけたぶん全員」と対応状況（各担当者が［📝 決定する］を押したか）を見られる
...
- **決まっているかの判定は `_renDecidedOf(w)` の1箇所だけ**＝ **更新ステータスが入っているか**。
  🧪 テストだけは決め直せるよう常に「未対応」。
  ⚠️ **🔁 繰り越しを未対応あつかいにしないこと**（決めても ⏳ 未対応 のままになり、
  「決めたのに消えない」と食い違う。実際に食い違った）
...
- サイドバー・マイページには、自分の担当ぶんのお知らせと**別に**
  「🌐 更新決定（全体）」を出す（`updateRenewalCycleAlerts` の中の1箇所。押すと `openRenCycleAll()`）
- ⚠️ **［📋 依頼の履歴と対応状況］は更新決定の一覧より上に出す**
  （`#renCycleHistModal` の z-index 1115。無いと一覧のうしろに開いて押せない。実際に隠れた）
...
- 入口は3つ＝ 更新決定の一覧の上と下のボタン、依頼をかけたあと
  - **対象の決め方は `REN_TARGET_MONTHS_AHEAD` まわりの一かたまりだけ**（`_renCycleTargets`）
    ...
    - ⚠️ **繰り越しでも、決めたら「残り」から外す**（`_renCycleUndecided` ＝
      更新ステータスが入っていない人材だけ）。以前は `w._carry` を無条件で残していたので、
      **担当者が決め直しても（サブ担当にお願いして決めてもらっても）お知らせが消えなかった**
      （実際に消えなかった）。繰り越しの行は**一覧には 🔁 の印つきで残る**ので、
      申請がまだ動いていないことは見える。まだ申請が動いていないこと自体は
      ⏰ 申請依頼まちが在留期限の75日前から拾う
...
- ⚠️ **`_renWriteStatus` は ⏰ 申請依頼まち と 🔁 更新決定 の両方を数え直す**（開いている
  更新決定の一覧＝`renCycleBody` も描き直す）。片方だけだと、決めたのにもう片方が古いまま残る
...
### 🎯 アラートの担当マーク（だれが処理するのか）

- アラートの**絵文字は使わない**。代わりに「丸いマークの中に担当の文字が入ったバッジ」を出す
- **区分・色・マークの文字は `ALERT_OWNERS` の1箇所だけ**
  ...
  - ⚠️ **「個人（名指し）」ぶんは、色を決めてから `ALERT_OWNERS` に1行足す**（いまは `all` にまとめてある）

- **どのアラートがどの担当かは `ALERT_OWNER_OF` の1箇所だけ**（書いていないものは `all`＝赤）
  - 🟢 GLT … 通算期間の精査状況／【要確認】通算期間不明／パスポート期限切れ／必須項目が未入力／
    面談記録の記入待ち／報告書の日付が足りない人材／脱退一時金 転出/転入手続き／退職予定日超過
    - ⚠️ もとは青（`#2563eb`）だったが、**枠線が薄くて見逃す**と言われて緑にした。
      色を選ぶときは**枠（`line` / `soft`）がはっきり見えるか**を必ず見ること
      （地の色だけ濃くしても締まらない）
  - 🟣 企業 … 更新決定（`renCycle` / `renCycleAll`）
  - 🟠 総務 … 申請忘れ警告／更新リマインド／在留期限の更新待ち／建設業 就労管理システム報告／申請依頼まち
- **マークを作るのは `_ownerMarkHtml(kind, size)` の1箇所だけ**（`alertOwnerMark(key, size)` が
  `ALERT_OWNER_OF` を引いてから呼ぶ）。`size` は `'lg'`（大きなバナー 38px）／省略（26px）／`'sm'`（サイドバー 17px）
  - 文字が長いもの（全・個）は**丸から横長のカプセルになる**（`border-radius:999px` ＋ `min-width`）。
    無理に丸に収めると読めなくなるため
- 🎨 **背景・枠・文字の色も担当の色にそろえる。読むのは `alertOwnerStyle(key)` の1箇所だけ**
  （マークだけ分けても、地の色がバラバラだと結局どれが自分の仕事か分からないため）。
  1件ぶんの色は `face`（地）／`face2`（グラデーションの下）／`line`（枠）／`soft`（細い枠）／
  `text`（見出し）／`sub`（説明文）／`shadow` の7つ
  - 📏 **アラートの箱は1種類だけ＝`alertRowCss(key)`（1行の細い箱）。**
    中身を作るのは **`alertRowHtml(key, 見出し, opt)` の1箇所**
    （`opt` は `sub`＝見出しの右の小さい説明／`count`＝右の大きい件数／`right`＝いちばん右）。
    右の大きい件数は **`alertRowCount(key, '14名')` の1箇所**（22px。これより大きくすると
    行の高さ48pxが変わってそろわなくなる）、右端の「クリックで一覧表示 ▼」は
    **`alertRowMore(key, text)` の1箇所**
    - ⚠️ **件数は見出しの中に「（◯名）」で書かない。** `count` に渡して**右側に大きい文字**で出す
      ＝どの行も同じ位置で件数が読める（GLTの行も総務の行も同じ）
    - ⚠️ **箱を2種類に増やさない。** 以前は「大きなバナー」（`alertBannerCss`＝2px枠＋影＋26pxの数字＋
      2段組み）があり、総務のアラートだけ背が高く・太く見えていた（実際に「GLTのサイズにそろえて」と言われた）
    - **件数は見出しの中に「（◯名）」で入れる**（GLTの行と同じ書き方）。右上に大きな数字を置かない
    - 🏗️ 建設業の3つのボタン（配属報告・退職報告・2号変更報告）は**見出しと同じ行の右端**に出す
      （下の段に置くとその箱だけ高くなる）
  - 🧹 **アラートの箱には説明文を出さない。**
    タイトルと件数だけにして、**クリックして一覧を開いたときだけ、一覧のいちばん上に説明文を出す**
    （メインのアラート欄をすっきりさせるため）。説明文を出すのは **`_alertListDescHtml(key, text)` の
    1箇所だけ**（色は `alertOwnerStyle(key)` を見るので担当の色とそろう）。
    人数など動く数字が入る説明文は、`renderWorkerAlerts` 側では計算しない＝**`toggle*AlertList()` の中で
    渡された `list` から数え直す**（バナーを消したので、そこにしか値を持たないため）
  - サイドバーのチップは `_alertChipHtml` に `owner` を渡すと**担当の色が優先される**
    （呼ぶ側が渡した bg / border / color は上書きされる）
...
- 📏 **マークの大きさは `ALERT_MARK_PX`(26) / `ALERT_MARK_PX_SM`(17) の1箇所だけ。全部そろえる**
  - ⚠️ **`min-width` ＋ 左右の余白にしない**。「GLT」（半角3文字）と「総務」（全角2文字）で
    幅が変わって**バラバラに見える**（実際に言われた）。**幅と高さを同じ数字で固定**し、
    余白を持たせない＝必ず同じ大きさの正円になる
- 🚫 **マークを出さないところ**＝ サイドバーの「自分の担当のアラート」の内訳（`updateMyAlertSummary`）と
  「🔔 全体アラート」。欄が小さくて読みにくいので、**担当は文字の色だけ**で分かるようにしてある
- 🔽 **「自分の担当のアラート」は開閉できる。既定はたたんだ状態**（見出しと件数だけ）。
  **判定は `_myAlertOpen()` の1箇所だけ**で、`localStorage`（`kmt_my_alert_open_<user_id>`）に覚える
  （この端末・このユーザーだけ）。切り替えは `toggleMyAlertSummary()`。
  開いたときの内訳を押すとマイページが開く（**見出しは開閉、中身はマイページ**と役割を分ける）
- サイドバーのチップは `_alertChipHtml(..., cls, owner)` の**9番目の引数**にキーを渡すとマークが付く。
  マイページ側の大きい表示は `alertOwnerMark(key)` を直接置く
- 出る場所は3つ＝ 左メニューのお知らせ ／ 外国人材の画面のアラート ／ マイページ
  （`updateMyAlertSummary` の内訳チップも同じマークを見る）
...
- 期限カード（特例期間中／1・2・3ヶ月以内）、脱退一時金、退職予定日超過、在留情報未入力は
  従来どおり一覧フィルターに追従する
- ⏳ **期限カードを赤く点滅させる条件は `ALERT_CARD_URGENT`（カードのキー → 判定 → 文章）の1箇所だけ**。
  カードを増やすときはここに1行足す。読むのは `_cardUrgent()` / `_cardUrgentText()` の2つだけ

  | カード | 判定 | 条件（申請が止まったまま残りわずか） |
  |---|---|---|
  | `a0` 特例期間中 | `_tokureiUrgent()` | 申請ステータスが `TOKUREI_URGENT_APPLY`（審査中・審査完了）のまま、特例期間の残りが `TOKUREI_URGENT_DAYS`(15)日以下 |
  | `a1` 1ヶ月以内 | `_visaPrepUrgent()` | 申請ステータスが `VISA_PREP_URGENT_APPLY`（申請準備中）のまま、在留期限の残りが `VISA_PREP_URGENT_DAYS`(15)日**未満** |

  - 特例期間を過ぎると在留の根拠が無くなり、申請準備中のまま期限を過ぎると特例期間にも入れないため、
    どちらも**残り日数がいちばん短い人**を先に出す
  - 枠に出す文章は **`_tokureiUrgentText()` / `_visaPrepUrgentText()` の1箇所ずつ**
    （「⚠️ 特例期間 あと◯日 の人材が ◯名 います」／「⚠️ 在留期限 あと◯日 の人材が ◯名 います」）。
    点滅は既存の `.due-over`
  - **一覧（`toggleAlertList`）も同じ `_cardUrgent()` を見る**ので、件数と中身が必ず一致する。
...
- 在職ステータスが **辞退/キャンセル** と **支援機関変更済み** の人材は全アラートの対象外。判定は `isAlertExcludedWorker(w)` の1箇所だけ。
  ...
  - 📦 **一覧でも【過去の人材】に入る**（`workerBucket()` が同じ `isAlertExcludedWorker()` を見る。下の節）
...
### ✅ 在職ステータスを変えたときの確認・記入のお願い

- 在職ステータス（`wf_status`）を変えると、**選択欄のすぐ下（`#wf_status_guide`）**に
  「そのとき確認・記入してほしい欄」を出す。押すとその欄まで移動して黄色く光る
  ＝ **人材情報にすでにある欄を使う**（記入欄を二重に持たない）
- **どの変更で何を出すかは `WORKER_STATUS_GUIDE` の1箇所だけ**

  | キー | 意味 |
  |---|---|
  | `to` | 変更後（必須。文字か配列） |
  | `from` | 変更前をしぼりたいときだけ書く（省略＝どれでも）。**`from` 付きが先に当たる** |
  | `msg` | お願いの1行 |
  | `fields` | 確認・記入してほしい欄（`id` は人材情報の入力欄／`l` は画面に出す名前） |
  | `chat:false` | 💬 チャットに送るボタンを出さない（**配属前 → 在職中 だけ**） |

  - **当てはめるのは `_wfStatusGuideOf(from, to)` の1箇所だけ**
  - ⚠️ **`id` は人材情報に実在するものだけ**を書く（無い id はふるい落とすので黙って消える）
  - ⚠️ 項目を足すときはここだけ直す（画面・自動移動・✅/⚠️ の印・チャットの文面が同時に追従する）
- **変更前は `_wfStatusOpened` の1箇所**（モーダルを開いたとき＝`_wfStatusGuideReset()` で入れ、
  💾 保存したあとは保存した値に進める）。**保存してもお願いは消さない**
  （保存してから 💬 チャットに送るため）
- ✅ / ⚠️ の印は **`_wfFieldFilled(id)` の1箇所**で見る。打った先から合わせるのは
  **`_bindWfStatusGuideFields()`（document に1回だけ付ける委任リスナー）**
- 欄まで移動して光らせるのは **`wfStatusGo(id)` の1箇所**（隠れている欄は先に出す）。
  ステータスを変えた直後は**最初の欄へ自動で移動する**
- 💬 **チャットに送るのは `sendWorkerStatusToChat()`**、文面は **`_wfStatusChatText()` の1箇所だけ**
  （氏名・企業・◯◯ → ◯◯・記入した日付・変更した人と日時）。`mentions` は全員＝ @全員
  - ⚠️ **未保存のときは先に知らせる**（画面の内容がそのまま送られるため）。
    送ったあとの［💬 チャットを開く］は `_pendingChatRoomId` → `showScreen('chat')` を通す

### 📦 外国人材の3区分（🤝 支援人材 ／ ➖ 支援外人材 ／ 📦 過去の人材）

- **どの枠かを決めるのは `workerBucket(w)` の1箇所だけ**（タブの人数も一覧の絞り込みも同じものを見る）
- 📦 過去に入るのは **`WORKER_PAST_STATUSES` の1箇所**（退職済(帰国)／退職済(転職)／
  支援機関変更済み／**辞退/キャンセル**）＋ **`isAlertExcludedWorker(w)` が true のぶん**
  （「辞退」「キャンセル」の**表記ゆれ**もまとめて拾う）。
  **アラートの対象外と必ず同じ顔ぶれ**になる＝「アラートに出ないのに支援人材に居る」が起きない
- ➖ 支援外は `status === '在職中（支援なし）'` か `kmt_support === 'KMT支援外'`
- **人材そのものは消えない**（枠が変わるだけ）。上のタブで 📦 を選べば今までどおり見られる
...
- 📋 **［📋 候補者から自動入力］（`wf_candidate_src` ＋ `fillWorkerFromCandidate()`）は
  この検索欄と同じ行**（タブのすぐ下）に置く。以前は「6. 人材情報」の途中にあって見つけにくかった
  ＝ **新しく登録するときに最初に使うもの**なので上に出してある（中身は変えていない）
...
- **この節だけ、基本情報タブの2段組み（左＝青／右＝緑）をまたいで「全幅」に出している。**
  右カラム（442px）の中に入れていたときは会社名が数文字しか見えなかったため。
  右カラムを10番の手前でいったん閉じ、`grid-column:1/-1` の箱に入れてある
- ⚠️ **日付の欄（指定書発行日・終了日）は 116px 以上にすること。**
  狭いと Chrome が年の欄を2桁に縮めてしまい、**西暦を4桁で入れられなくなる**
  （月・日は入るのに年だけ入らない状態になった。100px でそうなっていた）。
  ほかの日付の欄にも効くよう **`input[type="date"].form-input{min-width:112px}` の1箇所**で下限を決めてある
  - 全幅にしたので、1行に 初回/6年・会社名・申請・在留資格・日付2つ・詳細・✕ が全部おさまる
    （横スクロールの下限は `min-width:672px` の1箇所）
...
- ⚠️ **［← 戻る］は `history.back()` を呼ぶだけ**にして、実際の切り替えは **`popstate` の1箇所**でやる
  ＝ ボタンでもブラウザの戻るでもスマホの戻る操作でも**必ず同じ動き**になる
  （`pushState` が使えない環境＝`file://` などでは `_navHistoryOk` が false になり、
  ボタンが `_navGoPrev()` を直接呼ぶ）
- 🪟 **モーダルが開いているときは、まずそれを閉じる**（`_topOpenModalId()` の1箇所で
  いちばん上のものを探し、`closeGenModal()` に渡す＝✕ を押したのと同じ道なので未保存の確認も出る）。
  このとき**画面は変えず、履歴も減らさない**（押した戻るをその場で積み直す）
- ⚠️ **未保存のリマインドで `showScreen` が止まったときも履歴を積み直す**
  （`_navGoPrev()` が false を返す）。減らしっぱなしにすると次の戻るがずれる
- 🔁 **戻っている最中は積まない**（`_navPopping`）／**ログイン直後の1回目も積まない**
  （`_navStarted`。戻り先が無いため）
- ボタンの出し入れは **`renderNavBack()` の1箇所**（戻り先が無いときは出さない＝押せないボタンを置かない）。
  見た目は **`.nav-back-btn` の1箇所**で、スマホでは「戻る」の文字を消して ← だけにする
  - ⚠️ スマホでは ← のぶん狭くなるので、**画面の名前は折り返さず … にする**
    （`.topbar .page-title`。2行になるとヘッダーの高さが変わって下がずれる）

### 🔢 左メニューの見出しの通し番号

- 見出し（`.sidebar-section`）に **1. 2. 3. …** の通し番号を出す。
  **振るのは `initSidebarSections()` の1箇所だけ**で、**画面に並んでいる順そのまま**
  （見出しを足すと自動でずれる。**番号を手で書かないこと**）
- **番号を付けないのは `SIDEBAR_NO_NUM` の1箇所だけ**（いまは `設定`）。
  ⚙️ 設定は場所が決まっていて常にいちばん下なので、順番の番号にはしない
- ⚠️ **`data-section` に入れる名前（開閉の記憶やほかからの検索に使う）は番号なしのまま。**
  番号は見た目だけ（`.sidebar-section-no`）
...
- `applyExtFormLang()` が1箇所で全部やる
  ...
  - `.i18n-inline` … **小さな印**（［必須］［任意］のバッジ・タブの中）は `option` と同じで行を足せないので
    「必須 / Wajib」にその場で書き替える。**この1箇所だけ**
  - `_extJa(ja)` … 画面に足せない文字（`confirm` / `alert` / その場の知らせ）を「日本語 / 訳」にする。**この1箇所だけ**
- ⚠️ **`.form-label` を flex にしない。** 訳（`.i18n-tr`）が flex の1項目になって**横にならぶ**ので、
  バッジは**ラベルの前に置いたふつうの1行**にする（訳はラベルの下に改行して出る）
- ⚠️ **タブの見出しを `nowrap` / `ellipsis` にしない**（訳が切れて読めなくなる）
- 📣 募集リンクの応募フォームは**画面の日本語をひとつ残らず**対応表に入れてある
  （項目名・記入のしかたの説明・記入例の placeholder・印・タブ・ボタン・確認のメッセージ）。
  **新しい文字を足したら必ず `EXT_FORM_I18N` にも足すこと**
...
- ⚠️ **KMT案件だけが売上に入り、紹介案件（他社支援・企業単独）は大房に送るだけだった**＝
  🤝（紹）決定者リストにも 💰 売上にも入っていなかった（実際に入っていなかった）。
  いまは紹介案件も自動で入る

  | 決定報告 | 送り先 | 決定者リスト | 売上 |
  |---|---|---|---|
  | 🏢 KMT案件 | 依頼スプレッドシート | （入れない） | `saveDecisionInitialFeeToSales()`＝初期費用 |
  | 🤝 他社支援（紹介） | 大房 `intake_requests` | ✅ | ✅ 人材紹介料 |
  | 🏛️ 企業単独（人材紹介） | 大房 `intake_requests` | ✅ | ✅ 初期費用（内・申請費用5万円） |

- **フォーム → 行を作るのは `_drIntroRowsOf(kind)` の1箇所だけ**（1名につき1件。
  他社支援は人材名が複数行なので人数ぶん作る）
- **書き込むのは `linkIntroDecisionFromReport(kind)` の1箇所だけ**。入口は2つ＝
  ① ［📥 大房の受信トレイへ送信］の成功後（自動・`quiet`）
  ② フォームの［🤝 決定者リスト＋売上に登録］（テキストだけコピーして使うとき）
- **項目名・単位・内申請費用は `DR_INTRO_SALE` の1箇所だけ**＝
  他社支援＝`人材紹介料`／紹介料の欄は**円**、企業単独＝`初期費用`／初期費用の欄は**万円**（×10000）
  - ⚠️ **`人材紹介料` は `soshiki`（登録支援機関）が必須**（`saveSale` が止める）ので、
    他社支援は `ot_support_org` を必ず入れる
  - 受け入れ企業が KMT の所属機関に無いことがあるので、**見つからなければ
    `company_name_custom` に企業名を入れる**（`company_id` は null）
...
- 請求月度・年度（4月はじまり）・請求月・決定日付を作るのは **`_drIntroPeriod()` の1箇所**
  （もとは 📥 依頼受領日。空なら今日）。国内／国外は **`_drIntroDomestic()` の1箇所**
  ＝ 認定→国外／変更・更新→国内
- 同じ名前の候補者が**1人だけ**いれば `candidate_id` に結び付ける（`_drIntroCandId()`）

#### 🤝（紹）決定者リストの取り込み（`intro_decisions`）

- 埋め込みシード `INTRO_DECISION_SEED`（362件）を初回アクセス時にDBへ入れる
- ⚠️ **取り込みに失敗したことを黙って隠さない。** 以前は失敗すると埋め込みデータを
  そのまま画面に出すだけだったので、**DBは0件なのに画面には362名出ている**状態に
  気づけなかった（実際に気づけず、9月の決定が入らないという形で表に出た）。
  そのとき新しく決定者を足すと、次に開いたときに362名と入れ替わって**消えたように見える**
  - **保存されていない理由は `_idNotSaved` の1箇所**に持ち、`renderIntroDecisions()` が
    一覧の上に赤い帯と［🔄 取り込み直す］（`retryIntroDecisionSeed()`）を出す
- **取り込みは `seedIntroDecisions()` の1箇所**。**`ID_SEED_CHUNK`(20) 件ずつ**入れ、
  **1つのかたまりが失敗しても次へ進む**（何件入って何が失敗したかを出す）
  - すでに入っている `sort_order` は飛ばすので、**途中で止まっても続きから入れ直せる**
    ／何度押しても二重にならない
  - 全部入ったら **`app_settings.intro_decisions_seed = 'done'`** の印を付けて、
    次からは走らせない（**消した決定者が復活しないようにするため**）。
    判定は `_idSeedDone()`、印を付けるのは `_idSeedMark()`
...
- サイドバーの**全体アラート（`sidebarAlerts`）とアナウンス確認（`announcementAlerts`）の間**のボタン。
  押すとスプレッドシートを**新しいタブ**で開くだけ（`openRequestSheet()`）
- **リンクは `REQUEST_SHEET_URL` の1箇所だけ。** シートを差し替えるときはここを直す
  （現金小口の `PETTY_CASH_SHEET_URL` と同じ考え方）
- だれでも押せる（権限のチェックはしない）。シート自体の閲覧・編集権限は Google 側で決まる
...
- **どこまでやってもらうかは `ANN_STEP_PRESETS` ＋ `announcements.required_steps` の1箇所**
  （☑️ 確認だけ ／ ☑️ 確認 → 🏁 完了 ／ ☑️ 確認 → 🔧 対応中 → 🏁 完了）。
  段階を読むのは `_annSteps(a)`、いまの状態を決めるのは **`_annStateOf(c, a)` の1箇所だけ**で、
  **書いた段階を全部終えたときだけ `done`＝お知らせから消える**（社内共有事項の一覧には残る）。
  ⚠️ 「開いてすぐ確認だけ押される」のを防ぎたいアナウンスは `['confirmed','done']` にする
  （確認を押しただけではお知らせが消えない）
...
- 書類でフルネームにする欄は **`REG_DOC_FULLNAME_KEYS` の1箇所**（担当者名・説明者名が入る欄）。
  `staff` は 6・8・9・12・14・15 が共通で使うキーなので、**1つ書けば全部の様式に効く**
  - 🧾 **行（`type:'rows'`）の中の担当者は `REG_DOC_FULLNAME_ROW_KEYS` の1箇所**
    （フィールドのキー → 列のキー。相談記録書 5-4号の「対応者の氏名」など）
  - ⚠️ **人材・企業の名前が入る欄は入れない**（`org_name`＝機関名／`members[].name`＝外国人材／
    `sv_name`＝企業側の監督者／`co_rep`・`org_rep`・`agent_name`＝代表者／
    no.1 の `staff[].name`＝概要書の支援担当者〈ローマ字の列を別に持っている〉）。
    `fullNameOf()` は名字だけでも1人に絞れれば拾うので、社名や外国人材名を通すと取り違える
...
- 入管の実地調査（報告徴収・立入検査）にそなえるための読みものと書類のリンク。
  サイドバー「登録支援機関業務」の**いちばん上**（`renderRegSupportNav` の1箇所）
- **全員が見られる**（読みものと書類のリンクだけなので、権限で止めていない）。
  もとは管理者だけだった
- 🚫 **この画面にはアップデートのお知らせを出さない**（画面上部のバナーも、
  左メニューの赤い●も）。読みもの中心で「新着」を知らせる意味がないため。
  **出さない画面の一覧は `UPDATE_NO_BANNER_SCREENS` の1箇所だけ**で、
  `_updHitsScreen()` が必ず通るので **`screens:'all'` のエントリもここで止まる**
...
- **自動で入る欄は、フィールド定義の3つのキーの1箇所だけ**で決める
  ...
  | キー | 入る値 | 例 |
  |---|---|---|
  | `from` | `workers` の列 | 支援対象者管理簿（no.3）の 支援開始日＝`joined_date`（配属日）／支援終了日＝`left_date`（退職日）、出入国送迎記録簿（no.5）の `entry_date` など |
  | `fromCo` | `companies` の列（`from` と両方なら人材が空のときの控え） | 5-10号の委託料＝`fee_per_person` |
  | `fromFn(w, co, d)` | 計算して入れる | 5-10号の契約期間（終了）＝就労開始日の1年後 |

  - 📝 **全案件で同じ定型文を最初から入れたいときも `fromFn`**（人材を見ずに定数を返すだけ）。
    文章は**定数の1箇所だけ**に置く＝ 住居確保・生活契約支援記録簿（no.6）の「保証人契約の内容」＝
    `REG_6_GUARANTEE_TEXT` ／ 同「支援した契約の内容」＝ `REG_6_CONTRACTS_TEXT` ／
    関係機関同行等支援記録簿（no.8）の「実施内容」＝ `REG_8_CONTENT_TEXT`。
    入るのは**空の欄だけ**なので、案件ごとの事情はそのまま書き替えられる
    - ⚠️ **記録簿の定型文は必ず過去形（「〜しました。」）にそろえる。**
      実際に行った支援の記録なので、「〜を行う」だと予定に読める
  - 🔘 **選択式（`type:'select'`）の既定値も `fromFn`**。選択欄には必ず「-- 選択 --」が先頭に付くので、
    `opts` の並びでは既定にならない＝ **`fromFn:() => REG_8_METHODS[0]` のように値で入れる**
    （no.8 の実施方法＝訪問・同行／オンライン案内。**選択肢と既定は `REG_8_METHODS` の1箇所**で、先頭が既定）
  - 📍 **住所の一部を入れたいときは `_prefCity()` を通す**（no.8 の実施場所＝日本住所の都道府県＋市区町村）。
    「四日市市」のような地名も正しく切れる（自前で正規表現を書かない）
...
- **行の所属機関を引くのは `_regCoRow(x, mode)` の1箇所だけ**（企業ごとの様式＝`x.ent` ／
  人材ごとの様式＝その人材の所属機関 ／ どちらも無ければ記録の `company_id` から探す）。
  企業番号（`_regCoNoHtml`）も名前も同じものを見るので、番号と名前が食い違わない
  - ⚠️ **記録の中に企業名（`data.company_name`）を持っていない様式がある**（5・6・11・12・14 など）。
    `cell()` は `_company` の値が空のときに `_regCoRow()` から引き直す。
    これが無いと、**記入済みの行だけ所属機関が空**で出る（未記入の行には出るので余計に分かりにくい）
  - **`listCols` の先頭に `['_company','所属機関']` を足すだけ**でその様式に列が付く
    （いま付けてあるのは 2・3・4・5・6・7・9・10・11・12・13・14・15・16。
     付いていないのは 1〈記録のみ〉・8 だけ）
...
- 記録簿の画面（`buildRegSupportTaskScreen`）は**幅を制限しない**（`max-width` を付けない）。
  ⚠️ 以前は `max-width:1000px` だったので、列の多い様式（出入国送迎記録簿など）は
  **右側に余白があるのに一覧が切れていた**（実際に言われた）。
  表は `width:100%` ＋ `overflow-x:auto` なので、狭い画面では今までどおり横スクロールになる

#### 🔗 様式ごとのドライブ直通ボタン

- 一覧の上のボタン行に出す「その資料を開くだけ」のリンク。
  **足すのは `REG_TASK_LINKS` の1箇所だけ**（様式番号 → `[{ label, url, title, icon }]`）で、
  出すのは `_regTaskLinksHtml(no)` の1箇所。新しいタブで開くだけ（閲覧権限は Google 側）
  - `icon` は**省略すると 📁**（フォルダー）。スプレッドシートなど中身が違うものは
    `icon:'📊'` のように書き分ける（ドライブのフォルダーでないものに 📁 を出さない）
- いま入っているのは2つ
  | 様式 | ボタン |
  |---|---|
  | 2. 支援委託契約書（5-10号） | 📊 原本管理シート（Google スプレッドシート） |
  | 9. 日本語学習支援記録簿 | 📁 日本語学習連携合意書・実施報告書 |
- 全様式ぶんで1つの［📁 保存先フォルダー］（`REG_DOC_FOLDER_TASK`＝13）とは別もの。
  **様式ごとの資料はこちらに足す**

#### 🔤 一覧の並べ替えと 🗂 表示の切り替え（記録の一覧）

- **見出しを押すと並べ替え**（1回目 A→Z ／ もう一度で Z→A）。人材リストと同じ動き
  - 持つのは **`ui.sortKey` / `ui.sortDir` の1組だけ**（`_regUI(no)` の中）。切り替えは `setRegListSort(no, key)`、
    矢印は `_regSortArrow()`、見出しを作るのは `renderRegTaskList` の中の **`th()` の1箇所だけ**
    ＝ **`listCols` に列を足せば並べ替えもそのまま付いてくる**
  - **くらべる値は表に出しているもの（`cell()`）そのまま**なので、見えている文字の順にならぶ。
    ⚠️ そのため **`entLink` / `cell` の定義は件数チップより前に置いてある**（並べ替えが `cell` を使うため）
  - 所属機関の列は**企業番号を頭に付けて**くらべる（画面も番号から出しているので、見た目と必ず同じ順になる）
  - 状態の列（`_state`）は **`REG_STATE_RANK` の1箇所**の順（記入済 → 人材情報でそろい → 未記入 → 対象外）
    ＋ 最終更新。空欄は**向きにかかわらず必ず最後**（空が上に来ると何も見えないページができる）
- **🗂 表示するものは `REG_VIEW_KINDS` の1箇所だけ**（🗂 全体表示／✅ 記入済だけ／🟡 未記入だけ）。
  もとの「未記入も表示」のチェックはこれに置き換えた
  - ⚠️ **件数チップ（記入済 ◯件 / 未記入 ◯名）はしぼる前の数を出す**（しぼると全体で何件あるのか
    分からなくなるため）。そのため **`ui.showBlank` は常に true** にして未記入の行をいったん作り、
    そのあとでしぼる。作らないようにすると「未記入 0名」と出て、**片づいたように見える**
  - 0件のときは「＋ 新規作成から登録してください」ではなく、しぼった結果であることを出す

#### 📘 支援実施体制管理簿（no.1 ／ 表示も「1.」）

- ⚠️ **1件＝登録支援機関概要書の「1つの版」**（例「2026.1.29ver」）。人材にも企業にもひも付かない
  （`_regListMode` は `none`）。**作った管理簿は日付ごとに一覧に残る**
- **元データは `reg_org_profiles`**（1行＝1つの版。**読むのは `loadRegOrgProfiles()` の1箇所だけ**）＝
  参考様式第２－２号 登録支援機関概要書の中身。`ver`（版の日付）／`label`（版の名前）／`data`（中身）
  - 概要書 → 管理簿の中身に直すのは **`_reg1DataOf(p)` の1箇所**
    （`data.extra` の「5〜8欄」は「項目｜内容」の行＝`notes_rows` に直す）
  - 🏢 月次支援実績の【所属機関社数】（`monthly[].co`）は、概要書の
    **`REG_1_CO_COUNT_KEY`（`支援委託契約を締結している特定技能所属機関`）**から数字だけを取って入れる。
    **入れるのは `_reg1DataOf()` の1箇所だけ**で、**空いている行だけ**（手で直した数は残す）。
    ⚠️ 概要書の項目名を変えたら `REG_1_CO_COUNT_KEY` も直すこと
  - 📋 項目ごとの管理簿リスト（`renderReg1Panel`）も書類（`buildRegTaskDocHtml` の `no===1`）も、
    **`type:'rows'` の `cols` をそのまま並べる**ので、**列を足すのは `REG_FORM_DEFS[1]` の1箇所だけ**
    （画面・記入フォーム・書類が同時に追従する）
  - フォームに流し込むのは **`_reg1Apply(data, force)` の1箇所**（`force` 無しなら**空いている欄だけ**）
  - 🙋 支援責任者・支援担当者の列は 区分／氏名／**氏名（ローマ字）**／役職・住所／**役職（英語）**（就任日は未使用のため列から外した）。
    ⚠️ **氏名は `reg_support_records` と `reg_org_profiles` にだけ持ち、`kmt.html` には書かない**（個人情報）。
    ⚠️ **直すときは概要書（`reg_org_profiles`）のほうもそろえること**＝
    ［📥 登録支援機関概要書から取り込む］で作り直すと概要書の中身で上書きされるため
- **版の名前は `_regVerLabel(日付)` の1箇所**（`2026-01-29` →「2026.1.29ver」）。
  版の日付を直すと `reg1SyncVerLabel()` が名前の空欄に入れる
- 入口は画面の上の2つ
  | ボタン | すること |
  |---|---|
  | 📥 登録支援機関概要書から取り込む（`openRegOrgImport`） | 概要書の版を選ぶ → その版の管理簿を作る（**同じ版がすでにあれば開く**＝二重に作らない） |
  | 📋 新しい版を作る（`reg1NewVersion`） | いちばん新しい版をコピーして、今日の日付で新規（次の概要書を作るときの下地） |
- 【記録の一覧】は2段。**上＝📋 項目ごとの管理簿リスト**（`renderReg1Panel`。
  **出す項目は `REG_1_SECTIONS` の1箇所だけ**。上の `<select>` で版を選べる）／
  **下＝📄 書類作成の一覧（版ごと）**（ふつうの記録の一覧。列は `listCols`）
- 書類は版（作成日）を右上に出す。`_regDocPeriodLabel()` も版の名前を返すので、
  【📄 作成済みの書類】の「対象期間」に版が並ぶ
- いま入っている版は **2026.1.29ver ／ 2026.9.10ver** の2つ。
  新しい版を足すときは **`reg_org_profiles` に1行 ＋ `reg_support_records`（`task_no=1`）に1行**の2つを作る
  （管理簿の `data` は概要書の `data` から `extra` を `notes_rows` に直したもの＝`_reg1DataOf()` と同じ形）
...
- **相談記録の行は面談記録（`company_meetings`）から作る**＝ `reg54FillRows(force)` の1箇所
  ...
  - **定期面談・随時相談・その他を分けずに全部入れる**（所属機関の【面談記録】から入れても
    外国人材の【面談記録】から入れても同じ `company_meetings` なので、どちらもここに出る）。
    ⚠️ **`limit` を書かない**＝ `sbFetch` が最後まで読む（以前は `limit=50` で打ち切っていた）
  - 📝 **相談内容も対応結果も空の記録は入れない**（予定だけの枠が日付だけの行になって並ぶため）。
    **1件も書かれていないときだけ**、今までどおり日付のある記録を全部入れる（空にしない）
  - **項目名を頭に付けるのは `_reg54Content(m)` の1箇所だけ**＝ 随時相談は【随時相談】、
    その他は【電話】のように `kind_other` を出す。**定期面談には付けない**
    （いちばん件数が多く、全行が同じ印で埋まるため）
...
- **別紙は「１号特定技能外国人支援対象者名簿」**（参考様式第４－３号 別紙と同じ形）＝
  ...
  - 🧑 **人材を探すのは `_meetWorkerOfMember(o)` の1箇所だけ。**
    見る順番は **① `id` ② 在留カード番号 ③ その名前の人材が1人だけのときだけ名前**
    - ⚠️ **名前だけで探してはいけない。同姓同名が実際にいる**（8組）。
      > **過去の事故**: ［＋ 行を追加］で手で足した行は **氏名とカード番号しか持たない**
      > （`regAddRow` が `{}` を積むため）。`id` が無いので名前で探しており、
      > **同じ氏名の別人の生年月日・住居地が別紙に出ていた**（2026/09/03 に指摘された）
    - ⚠️ **見つからないときは空のまま返す**（別人の値を借りない）。
      空欄なら気づけるが、別人の値は気づけない
  - 📥 **保存のときに人材情報の中身を記録へ書き戻す**（**`_reg13FixMembers(d)` の1箇所**。
    `saveRegTaskRecord` の `no===13` から呼ぶ）＝ そのあとは人材が消えても・同姓同名が増えても、
    **記録だけで別紙が正しく出る**
  - ⚠️ 人材情報に当たらない行は、記入フォームに黄色の注意を出す
    （**`_reg13MemberWarnHtml()` の1箇所**。`regRenderRows` の末尾から呼ぶ）
  - 住居地は `workers.jp_address` ＋ 電話（`phone` → `cel_phone`）。
    読む列は `REG_WORKER_SELECT_COLS` に入れてある
- 🙋 **２① 対応者の氏名は複数書ける**（国籍ごとに担当が分かれている企業があるため。
  例: インドネシアとタイの人材がいる企業 → 「アヒュ、ガイ」）。
  **区切り方も、書類のフルネームにするのも `MEET_STAFF_SEP` / `_meetStaffFull()` の1箇所だけ**
  （書類では「・」でつなぐ）。⚠️ **空白では区切らない**（「神出　紘陽」のように氏名に空白が入る）
...
- **実ファイルは Google ドライブに入れる**（チャットの添付・領収書と同じ GAS 経由）。
  **置き場は `REG_DOC_DRIVE_URL` の1箇所だけ**、入れるのは `_regUploadDocToDrive()` の1箇所だけ
  - 📁 **［保存先フォルダー］のボタンを出す様式は `REG_DOC_FOLDER_TASK`（13）の1箇所だけ**＝
    **13. 定期面談報告書のメインページ**（画面のいちばん上のボタン行）。
    フォルダーは全様式ぶんで1つなので、各様式の【📄 作成済みの書類】には置かない
...
- 🧑 **フルネームで入れたいときは `fromFn` に `fullNameOf()` を通す**
  （住居確保・生活契約支援記録簿（no.6）の「実施担当者氏名・役職」＝
  `fromFn:(w, co) => fullNameOf(co?.main_staff)`）。
  所属機関の担当欄は「白井」のように**名字だけ**のことがあるが、`_userByName()` が1人に絞れるときだけ拾う。
  ⚠️ `fullNameOf()` は**フルネームを渡しても同じ値を返す**ので、`REG_DOC_FULLNAME_KEYS` に入っている欄
  （`staff` など）に使っても二重変換にはならない
...
- 🪪 **人材の候補は「人材番号　氏名」で出し、人材番号順にならべる**
  （請求先が【人材】のピッカーも、【企業】【登録支援機関】の行の候補も同じ）
  - **並べるのは `_expSortWorkers()` の1箇所**（番号は `000-001` と桁がそろっているので文字のままくらべる。
    **番号が入っていない人材は末尾に氏名順**）
  - **候補・入力欄に出す文字は `_expOptLabel()` の1箇所**（企業は今までどおり企業名だけ）。
    ⚠️ **保存する名前に番号は入れない**＝ `updateExpRow()` が「番号　氏名」で選ばれたときも
    `name` には氏名だけを入れる（一覧・検索に使う `_expPairName()` と必ずそろう）
  - 番号を打っても氏名を打っても候補が絞れる（datalist の値が「番号　氏名」なので）
...
- 絞り込みは5つ（**状況（未作成／未請求／未入金）** ／ 項目 `saleItemFilter` ／ 支払方法 ／ 請求方法 ／ 請求法人）。
  絞り込みが1つでも効いていると見出しが「絞り込みの合計」になる
- ✅ **状況の絞り込みは `SALE_TODO_KINDS` の1箇所だけ**（表の 作成／請求／入金 と同じ列を見る）。
  えらんだものが**すべて「まだ」**の行だけを出す（2つ以上えらぶと「どちらもまだ」になる）
