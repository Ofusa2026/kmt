...
- 入口は2つで**同じ状態を指す**＝ 就労先の行の「6年」（`r.six`。**1行だけ**。`setTokuteiSix()`）と、
  計算結果の下のチェック欄（`wf_tokutei_6y`。`onTokutei6yChange()`）。
  **書き込むのはこの2つの関数だけ**で、行を押せばチェック欄が入り、チェック欄を外せば行の印も外れる。
  行の印は「どの申請で認められたか」の記録として残す（保存されるのは `tokutei_six_year` の1つ）
