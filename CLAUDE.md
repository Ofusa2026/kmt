...
- 💬 **発言の削除は論理削除**（`deleted_at`）＝ 一覧には「🗑️ このメッセージは削除されました」と残す。
  消せるのは**自分の発言だけ**
- ✏️ **発言の編集は管理者だけ**（［✏️ 編集］→ その場の入力欄 →［💾 保存］／Ctrl+Enter）。
  **できるかどうかは `_tcCanEdit(m)` の1箇所だけ**＝ `isAdminUser()` で、かつ
  **自動メッセージ（`team_chat_messages.is_auto`＝グループの説明など）か自分の発言**。
  自動メッセージは**だれの名前で入っていても管理者なら直せる**（いまは総務の2グループの説明に `is_auto` を付けてある）。
