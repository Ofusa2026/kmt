...
- **書類を組み立てる入口は `buildJobSheetDoc(j)` の1箇所**（分類を見て振り分ける）。
  ...
  - 🎓📋 **高度人材・一般は特定技能と同じ2列組み・同じ見た目**
    （`buildJobOrderSheet` の cell / sec とそろえてある。項目名はグレーの帯、記入欄はその右。
    番号は小さく前に、◎●○△の印はラベルの後ろ。列を増やすと窮屈で読みにくいため）。
    紙への収まりを決めるのは **`JOB_SHEET_DOC_COLS`（2）と `JOB_SHEET_COL_PX`（1列に入る高さ px）の2つだけ**
    - ⚠️ **行の高さは px で見積もる**（行数で数えると2行の行を2倍に数えてしまい実際と合わない）。
      1行＝`JS_GAP` ＋ max(`JS_ROW_MIN`, 行数×`JS_LINE_PX` ＋ 英語の行×`JS_EN_PX`)
    - ⚠️ **折り返しの字数は実測に合わせる**（記入欄の中身は約208px・文字は約10px なので19字）。
      甘く見るとページからはみ出す（実際にはみ出した）
    - ⚠️ **記入欄には `line-height` を必ず書く**。書かないとブラウザの既定（日本語フォントで約2倍）
      になり、2行のつもりが3行ぶんの高さになる（実際になった）
    - この見た目だと1行が約20pxなので、**いまの項目数（一般78・高度人材75）ではA4縦2枚**。
      1枚に入るのは**60項目前後**（英語併記だと45項目前後）。
      🎓 高度人材の**書類には英語の項目名を出さない**（`JOB_SHEET_DOC_EN` の1箇所。
      入力画面と参考用には出る）＝ 英語を出すと2枚では収まらないため
    - **詰め方は2段構え**（`fill()` の1箇所）＝ ①まず `JOB_SHEET_COL_PX` まで目いっぱい詰めて
      **必要なページ数を決める** → ②**最後のページだけ**、残りを左右の列で均等に分ける。
      ⚠️ ぜんぶ均等に割ると**どのページも下に大きな余白**が残って見た目が悪い（実際に残った）。
      ⚠️ 均等に割ると列が増えて1枚増えることがあるので、そのときは詰め直す
    - 見出しだけが列の最後に残らないよう、**次の1行まで入るか**を見てから置く
    - 列の幅は **`COLW` の1箇所**（番号 / 項目名 / 印 / 記入欄）。**記入欄を項目名より広くする**。
      日英併記（高度人材）だけ項目名を少し広くとる。折り返しの目安（`JS_LINE_*`）もこの幅から決める
