...
- 実体は `expenses.receipts` / `card_payments.receipts`（jsonb の `[{url,name,mimeType,size,at,by}]`）。
  **Drive に入れるのは `_uploadReceiptToDrive()` の1箇所だけ**（GAS 経由。両方ここを通る）
  ...
  - 🔧 うまくいかないときの切り分けは **`testReceiptDrive()` / `_receiptProbe()` の1箇所**
    （領収書の欄の［🔧 保存先をためす］）。いまの保存先と、**すでに動いているフォルダ**
    （`JOB_PHOTO_DRIVE_URL`）の両方に小さなテキストを送り、ドライブの返事をそのまま出す。
    「保存先だけ失敗」＝そのフォルダの問題／「どちらも失敗」＝GAS そのものの問題
  - ⚠️ **「アクセスが拒否されました: DriveApp。」の正体はフォルダの共有のしかた。**
    ...
    **フォルダ自体が「リンクを知っている全員（編集者）」だと、ファイルがその共有を受け継いでしまい、
    ...
    直し方は **フォルダの共有を［制限付き］にして、必要な人を個別に足す**。
    うまく動いているフォルダ（求人票の写真・チャットの添付）はどれもこの形
