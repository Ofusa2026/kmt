...
- **退職済で「直近1年（今月を含む12か月）」に何も入っていない人材は表に出さない。**
  判定は **`fmHasRecentRecord()` / `_fmRetiredNoRecord()` の1箇所だけ**
  - 「入っている」＝手入力（`_fmOvHasValue`。金額でも文字でも）があるか、実績（`FEE_ACTUALS`）の
    金額があること。**予定（自動計算）は入力ではないので数えない**
  - 外した人数は `#feeMgmtHiddenNote` に出し、［表示する］（`toggleFmRetiredEmpty`）でその場で確認できる。
    **黙って消さない**（なぜ居ないのか分からなくなるため）
  - 「除外者を表示」がONのときは、この自動除外は掛けない（手動除外者だけを見る画面のため）
